...
#define RTMP_ASIC_INTERRUPT_DISABLE(_pAd)		\
	do {			\
	HIF_IO_WRITE32((_pAd->hdev_ctrl), MT_INT_MASK_CSR, 0); /*0: disable*/\
	RTMP_CLEAR_FLAG((_pAd), fRTMP_ADAPTER_INTERRUPT_ACTIVE);		\
} while (0)
#define RTMP_ASIC_INTERRUPT_ENABLE(_pAd)\
	do {				\
		HIF_IO_WRITE32((_pAd->hdev_ctrl), MT_INT_MASK_CSR, (_pAd)->PciHif.IntEnableReg);     /* 1:enable */	\
...
#define RTMP_IRQ_ENABLE(_pAd)	\
	   ...
	   /* clear garbage ints */\
	   HIF_IO_WRITE32((_pAd->hdev_ctrl), MT_INT_SOURCE_CSR, 0xffffffff); \
