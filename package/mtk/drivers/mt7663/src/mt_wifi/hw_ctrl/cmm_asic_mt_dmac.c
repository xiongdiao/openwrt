...
INT MtAsicSetPreTbtt(RTMP_ADAPTER *pAd, BOOLEAN bEnable, UCHAR HwBssidIdx)
{
	...
	if (bEnable == TRUE) {
		 ...
		 */
		MAC_IO_READ32(pAd->hdev_ctrl, LPON_PISR, &timeout);
		timeout &= (~bitmask);
		timeout |= (0x50 << (HwBssidIdx * 8));
		MAC_IO_WRITE32(pAd->hdev_ctrl, LPON_PISR, timeout);
...
static INT MtAsicWtblWriteRateInfo(RTMP_ADAPTER *pAd, UINT8 wlan_idx, UINT32 *val)
{
	...
	if (IsMtAsicWtblIndirectAccessIdle(pAd) == TRUE) {
		HW_IO_WRITE32(pAd->hdev_ctrl, WTBL_ON_RIUCR0, val[0]); /* DW 5 */
		HW_IO_WRITE32(pAd->hdev_ctrl, WTBL_ON_RIUCR1, val[1]); /* DW 6 */
		HW_IO_WRITE32(pAd->hdev_ctrl, WTBL_ON_RIUCR2, val[2]); /* DW 7 */
		HW_IO_WRITE32(pAd->hdev_ctrl, WTBL_ON_RIUCR3, val[3]); /* DW 8 */
	...
	UINT32 EnableFeature)
{
	...
	if (Active) {
		...
		MAC_IO_WRITE32(pAd->hdev_ctrl, own_mac_reg_base + (OwnMacIdx * 8), val);
		val = OwnMacAddr[4] | (OwnMacAddr[5]<<8) | (1 << 16);
		MAC_IO_WRITE32(pAd->hdev_ctrl, (own_mac_reg_base + 4) + (OwnMacIdx * 8), val);
	}
	...
	if (OwnMacIdx < HW_BSSID_MAX) {
		if (Active) {
			val = (UINT32)((Bssid[0]) |
						   ...
						   (UINT32)(Bssid[2] << 16) |
						   (UINT32)(Bssid[3] << 24));
			MAC_IO_WRITE32(pAd->hdev_ctrl, curr_bssid_reg_base + (OwnMacIdx * 8), val);
			val = (UINT32)(Bssid[4]) | (UINT32)(Bssid[5] << 8) | (1 << 16);
			MAC_IO_WRITE32(pAd->hdev_ctrl, (curr_bssid_reg_base + 4) + (OwnMacIdx * 8), val);
		}
	}
...
INT32 MtAsicUpdateBASessionByDriver(RTMP_ADAPTER *pAd, MT_BA_CTRL_T BaCtrl)
{
	...
	if (BaCtrl.BaSessionType == BA_SESSION_RECP) {
		...
		if (BaCtrl.isAdd) {
			...
			MAC_IO_WRITE32(pAd->hdev_ctrl, (BSCR0_BAND_0 + band_offset), value);
			MAC_IO_READ32(pAd->hdev_ctrl, (BSCR1_BAND_0 + band_offset), &value);
			...
			MAC_IO_WRITE32(pAd->hdev_ctrl, (BSCR1_BAND_0 + band_offset), value);
		}
	} else {
		NdisZeroMemory((UCHAR *)(&ent), sizeof(struct wtbl_entry));
		if (mt_wtbl_get_entry234(pAd, BaCtrl.Wcid, &ent) == FALSE) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s: Cannot found WTBL2/3/4 for BaCtrl.Wcid(%d)\n",
		...
		if (BaCtrl.isAdd) {
			...
			if (32 - reg_bit >= 12) {
				HW_IO_READ32(pAd->hdev_ctrl, reg, &value);
				value &= (~(SN_MASK << reg_bit));
				value |= ((BaCtrl.Sn & SN_MASK) << reg_bit);
				HW_IO_WRITE32(pAd->hdev_ctrl, reg, value);
			} else {
				ASSERT(reg2 != 0);
				/* low bits */
				HW_IO_READ32(pAd->hdev_ctrl, reg, &value);
...
VOID MtAsicUpdateRxWCIDTableByDriver(RTMP_ADAPTER *pAd, MT_WCID_TABLE_INFO_T WtblInfo)
{
	...
	if (1) {
		...
		for (i = 0; i < 29; i++) {
			...
			HW_IO_READ32(pAd->hdev_ctrl, wtbl_addr, &wtbl_val[i]);
		}
...
INT MtAsicResetWPDMARGU(RTMP_ADAPTER *pAd)
{
	...
	if (IS_MT7663(pAd)) {
		UINT_32 u4value;
		MAC_IO_READ32(pAd->hdev_ctrl, MCU_CIRQ_IRQ_SEL4, &u4value);
		u4value &= ~(BIT1);
		MAC_IO_WRITE32(pAd->hdev_ctrl, MCU_CIRQ_IRQ_SEL4, u4value);
...
INT MtAsicInitWPDMA(RTMP_ADAPTER *pAd)
{
	struct _RTMP_CHIP_OP *ops = hc_get_chip_ops(pAd->hdev_ctrl);
...
VOID MtAsicSetExtMbssMacByDriver(RTMP_ADAPTER *pAd, INT idx, VOID *wdev_void)
{
	...
	MAC_IO_WRITE32(pAd->hdev_ctrl, cr_base_0, cr_val);
	cr_val = 0;
	cr_val = pmac_addr[4] | (pmac_addr[5] << 8);
	MAC_IO_WRITE32(pAd->hdev_ctrl, cr_base_1, cr_val);
}
...
INT set_get_fid(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8024, value);
	RTMP_IO_READ32(pAd->hdev_ctrl, 0x8024, (UINT32 *)&head_fid_addr);/* get head FID. */
	...
	while (1) {
		for (dw_idx = 0; dw_idx < 8; dw_idx++) {
			RTMP_IO_READ32(pAd->hdev_ctrl, ((MT_PCI_REMAP_ADDR_1 + (((value & 0x0fff0000) >> 16) * 128)) + (dw_idx * 4)), (UINT32 *)&dw_content);/* get head FID. */
		...
		RTMP_IO_WRITE32(pAd->hdev_ctrl, 0x8028, value);
		RTMP_IO_READ32(pAd->hdev_ctrl, 0x8028, (UINT32 *)&next_fid_addr);/* get next FID. */
...
INT MtAsicAMPDUEfficiencyAdjustbyFW(struct _RTMP_ADAPTER *ad, UCHAR wmm_idx, UCHAR aifs_adjust)
{
	HW_SET_PART_WMM_PARAM(ad, wmm_idx, WMM_AC_BE, WMM_PARAM_AIFSN, aifs_adjust);
	return 0;
}
