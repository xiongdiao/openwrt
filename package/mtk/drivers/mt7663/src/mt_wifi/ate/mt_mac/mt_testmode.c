...
INT MtATESetCleanPerStaTxQueue(RTMP_ADAPTER *pAd, BOOLEAN sta_pause_enable)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: StaPauseEnable:%d, StaID:%d, Band:%d, Reserved[0]:%d\n", __func__,
...
VOID MtATETestModeTxControl(RTMP_ADAPTER *pAd, UCHAR ubandIdx, BOOLEAN test_mode)
{
	UINT32 tmac_tcr = TMAC_TCR, tmac_tcr_val;

	if (ubandIdx == TESTMODE_BAND1)
		tmac_tcr = TMAC_TCR1;
	else
		tmac_tcr = TMAC_TCR;

	if (test_mode == TRUE) {
		MAC_IO_READ32(pAd->hdev_ctrl, tmac_tcr, &tmac_tcr_val);
		tmac_tcr_val &= ~(TBTT_TX_STOP_CONTROL | CCA_RELD_AIFS);
		MAC_IO_WRITE32(pAd->hdev_ctrl, tmac_tcr, tmac_tcr_val);
	} else {
		MAC_IO_READ32(pAd->hdev_ctrl, tmac_tcr, &tmac_tcr_val);
		tmac_tcr_val |= (TBTT_TX_STOP_CONTROL | CCA_RELD_AIFS);
		MAC_IO_WRITE32(pAd->hdev_ctrl, tmac_tcr, tmac_tcr_val);
...
#if defined(MT7663)
VOID MtATETestModePdmaWaitIdle(RTMP_ADAPTER *pAd, UINT8 ucPdmaIdx, UINT32 wait_time, UINT32 wait_round)
{
	UINT32 cpu_index, dma_index, i4LoopCnt = 0;
	PCI_HIF_T *hif = hc_get_hif_ctrl(pAd->hdev_ctrl);

	switch (ucPdmaIdx) {
	case HIF_TX_IDX1:
	    do {
			HIF_IO_READ32(pAd->hdev_ctrl, hif->TxRing[HIF_TX_IDX1].hw_cidx_addr, &cpu_index);
			HIF_IO_READ32(pAd->hdev_ctrl, hif->TxRing[HIF_TX_IDX1].hw_didx_addr, &dma_index);

			/* PDMA is empty */
			if (cpu_index == dma_index)
				return;

			msleep(wait_time);
	    } while ((i4LoopCnt++) < wait_round);

		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
		("%s: PDMA is not empty after 10sec \t cpu_index = %d, dma_index=%x\n", __func__, cpu_index, dma_index));
...
#if defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT)
VOID MT_ATEConcurrentTXCtrl(RTMP_ADAPTER *pAd)
{
	UINT band;

	for (band = TESTMODE_BAND0; band <= TESTMODE_BAND1; band++)
		MtATESetMacTxRx(pAd, ASIC_MAC_RX, TRUE, band);
}

UINT MT_ATEUpdateConcurrentStatus(RTMP_ADAPTER *pAd, UINT32 band_idx, UINT ate_mode)
{
	struct _ATE_CTRL *ate_ctrl = &pAd->ATECtrl;
	UINT uConcurrent_tx = 1, uConcurrent_rx = ATE_RX_INATIVE;
	UINT band, mode;

	switch (ate_mode) {
	case ATE_TXFRAME:
		for (band = TESTMODE_BAND0; band <= TESTMODE_BAND1; band++) {
			mode = TESTMODE_GET_PARAM(ate_ctrl, band, Mode);
			if (!(mode & ATE_TXFRAME)) {
				uConcurrent_tx = 0;
				break;
			}
		...
		/* if (uConcurrent_tx && IS_MT7626(pAd)) */
			MT_ATEConcurrentTXCtrl(pAd);

		ate_ctrl->isConcurrentTx = uConcurrent_tx;
		return ate_ctrl->isConcurrentTx;
	...
	case ATE_RXFRAME:
		for (band = TESTMODE_BAND0; band <= TESTMODE_BAND1; band++) {
			...
			if (!(mode & ATE_RXFRAME)) {
				...
				if (uConcurrent_rx == ATE_RX_INATIVE)
					uConcurrent_rx = ATE_RX_SINGLE_BAND;
				else if (uConcurrent_rx == ATE_RX_SINGLE_BAND)
					uConcurrent_rx = ATE_RX_DUAL_BAND;

			}
		}

		ate_ctrl->isConcurrentRx = uConcurrent_rx;
		return ate_ctrl->isConcurrentRx;
...
#endif /* defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT) */
...
static INT MT_ATETxHandler(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	if (IS_MT7615(pAd) || IS_MT7622(pAd) ||	IS_P18(pAd) || IS_MT7663(pAd) || IS_AXE(pAd) || IS_MT7626(pAd)) {
		...
		hwq_idx = asic_get_resource_idx(pAd, wdev, TX_MGMT, q_idx);
	}
#endif
...
round_tx:
		...
		if (((pkt_tx_time > 0) || (ipg > 0)) &&
			(pAd->mgmt_que.Number >= MGMT_QUE_MAX_NUMS)) {
				MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				("\033[32m%s:: band:%d, pkt_tx_time:%d, ipg:%d, Number:%u\033[0m\n",
				__func__, band_idx, pkt_tx_time, ipg, pAd->mgmt_que.Number));
			break;
		}
#endif /* defined(MT7615) || defined(MT7622) */
...
#if defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT)
			if (ate_ctrl->isConcurrentTx) {
				if ((free_token_cnt > (pkt_tx_token_id_max - ATE_ENQUEUE_PACKET_NUM)) && (free_num > 0)) {
					if (band_idx == 0 && pAd->mgmt_que.Number < MGMT_QUE_MAX_NUMS/2 - 5)
						goto round_tx;
					else if (band_idx == 1 && pAd->mgmt_que.Number < MGMT_QUE_MAX_NUMS - 5)
						goto round_tx;
				}
			} else {
				if ((free_token_cnt > (pkt_tx_token_id_max - ATE_ENQUEUE_PACKET_NUM)) && (free_num > 0)) {
...
VOID MT_ATEUpdateRxStatistic(RTMP_ADAPTER *pAd, enum _TESTMODE_STAT_TYPE type, VOID *data)
{
	...
	if (type == TESTMODE_RXV) {
		...
		if (RXV1_2ND_CYCLE->RxValidIndicator &&
			...
			pAd->u4RxMuPktCount++;
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO,
				("%s: GroupId:%d get MU packet #:%d\n",
				__func__, RXV1_2ND_CYCLE->GroupId, rx_stat->RxMacMuPktCount));
		}
...
static INT32 mt_ate_off_ch_scan(RTMP_ADAPTER *pAd, ATE_OFF_CH_SCAN *param)
{
	...
	work_tx_strm_pth = TESTMODE_GET_PARAM(ate_ctrl, ctrl_band_idx, tx_strm_num);
	work_rx_strm_pth = TESTMODE_GET_PARAM(ate_ctrl, ctrl_band_idx, rx_strm_pth);
	prim_ch[off_ch_wrk_ch_idx] = TESTMODE_GET_PARAM(ate_ctrl, ctrl_band_idx, Channel);
	bw[off_ch_wrk_ch_idx] = TESTMODE_GET_PARAM(ate_ctrl, ctrl_band_idx, BW);
	...
	for (off_ch_idx = 0; off_ch_idx < off_ch_ch_idx_num; off_ch_idx++) {
		...
		default:
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: off_ch_idx %d, BW is invalid %d\n",
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: mntr_ch:%d mntr_bw:%d mntr_central_ch:%d\n",
		__func__, ext_cmd_param.mntr_prim_ch, ext_cmd_param.mntr_bw, ext_cmd_param.mntr_cntrl_ch));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: work_prim_ch:%d work_bw:%d work_central_ch:%d\n",
		__func__, ext_cmd_param.work_prim_ch, ext_cmd_param.work_bw, ext_cmd_param.work_cntrl_ch));

	ret = mt_cmd_off_ch_scan(pAd, &ext_cmd_param);
	return ret;

err0:
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: NULL entry ate_ctrl %p, param %p, or invalid BW %d\n",
			__func__, ate_ctrl, param, bw[off_ch_idx]));
	return ret;
...
INT32 MT_ATEInsertLog(RTMP_ADAPTER *pAd, UCHAR *log, UINT32 log_type, UINT32 len)
{
	...
	return ret;
err0:
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("[WARN]%s: idx:%x, overwritable:%x, log_type:%08x\n",
			  __func__, log_cb->idx, log_cb->overwritable,
			  log_type));
err1:
...
static INT32 MT_ATEGetDumpRXV(struct _RTMP_ADAPTER *pAd, PINT32 pData, PINT32 pBufferLen)
{
#define MAX_DUMP_SIZE_OUT_RXV 40
	INT32 ret = 0;
	static INT32 idx;
	static INT32 remindIdx;
	INT32 i = 0;
	struct _ATE_CTRL *ATECtrl = &(pAd->ATECtrl);
	struct _ATE_LOG_DUMP_CB *log_cb = &ATECtrl->log_dump[ATE_LOG_RXV - 1];
	INT32 (*dump_func)(struct _ATE_LOG_DUMP_ENTRY, INT idx) = MT_ATEDumpRXV;
	INT32 end = 0;

	/*Get RXV dump from log dump*/
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("RXV dump from log dump log_cb->idx = %d, log_cb->len = %d\n", log_cb->idx, log_cb->len));

		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			("%s: ATECtrl->firstRXV %d\n", __func__, ATECtrl->firstRXV));

	/* start of scanning sub-buffer */
	if ((ATECtrl->firstRXV == TRUE) || (ATECtrl->firstQATool == TRUE)) {
		if (log_cb->is_overwritten) {
			idx = log_cb->idx;
			remindIdx = log_cb->len;
		} else {
			idx = 0;
			remindIdx = log_cb->idx;
		}
	}

	/* end of the scanning sub-buffer*/
	end = (remindIdx > MAX_DUMP_SIZE_OUT_RXV) ?
		((idx + MAX_DUMP_SIZE_OUT_RXV) % (log_cb->len)) :
		((idx + remindIdx) % (log_cb->len));

	/* the length of scanning sub-buffer */
	*pBufferLen = (remindIdx > MAX_DUMP_SIZE_OUT_RXV) ?
		MAX_DUMP_SIZE_OUT_RXV :
		remindIdx;

	/* remain length of the umdumping buffer */
	remindIdx = (remindIdx > MAX_DUMP_SIZE_OUT_RXV) ?
		(remindIdx - MAX_DUMP_SIZE_OUT_RXV) :
		(0);
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("end %d, remindIdx: %d *pBufferLen: %d \n", end, remindIdx, *pBufferLen));

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RXV DUMP START]\n"));

	if (*pBufferLen) {
		do {
			idx = (idx % (log_cb->len));
			if (log_cb->entry[idx].un_dumped) {
				struct _ATE_RXV_LOG rxv = log_cb->entry[idx].log.rxv;

				pData[i] = *((UINT32 *)&rxv.rxv1_1st);
				pData[i + 1] = *((UINT32 *)&rxv.rxv1_2nd);
				pData[i + 2] = *((UINT32 *)&rxv.rxv1_3rd);
				pData[i + 3] = *((UINT32 *)&rxv.rxv1_4th);
				pData[i + 4] = *((UINT32 *)&rxv.rxv1_5th);
				pData[i + 5] = *((UINT32 *)&rxv.rxv1_6th);
				pData[i + 6] = *((UINT32 *)&rxv.rxv2_1st);
				pData[i + 7] = *((UINT32 *)&rxv.rxv2_2nd);
				pData[i + 8] = *((UINT32 *)&rxv.rxv2_3rd);
				i = i + 9;
				/* Print log */
				dump_func(log_cb->entry[idx], idx);
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[After RXV dumping] idx: %d, end: %d\n", idx, end));
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, (" idx = %d, log_cb->idx = %d\n", idx, log_cb->idx));

		if (idx == log_cb->idx)
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[RXV DUMP END] : Count = %d\n", *pBufferLen));
		}
...
INT MT_ATERxDoneHandle(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk)
{
...

	{
		...
		case RMAC_RX_PKT_TYPE_RX_NORMAL:
			MAC_IO_READ32(pAd->hdev_ctrl, RMAC_CHFREQ0, &chfreq0);
			break;
...
#ifdef ATE_CONCURRENT_SUPPORT
	TESTMODE_SET_PARAM(ATECtrl, band_idx, wcid_ref, 1);
#endif
...
static INT32 MT_ATEStart(RTMP_ADAPTER *pAd)
{
	...
	if (IS_ATE_DBDC(pAd))
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s: wdev_idx:%u\n", __func__, Info->wdev_idx));
#endif /* DBDC_MODE */
#if defined(ATE_CONCURRENT_SUPPORT)
	MT_ATEStopTXFromUpperLayer(pAd);
#endif
	return Ret;
...
INT mt_ate_set_tmac_info(RTMP_ADAPTER *pAd, TMAC_INFO *tmac_info, UINT32 band_idx)
{
	...
	if (IS_MT7615(pAd) || IS_MT7622(pAd) ||	IS_P18(pAd) || IS_MT7663(pAd) || IS_AXE(pAd) || IS_MT7626(pAd)) {
		if (ant_sel & ATE_ANT_USER_SEL) {
			...
			if (control_band_idx == 0) { /* For Band 0 */
				if (ATECtrl->eTxBf || ATECtrl->iTxBf) {
					tmac_info->AntPri = 0;
					/* MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("AntPri = 0 for TxBF\n")); */
				}
			}
#ifdef DBDC_MODE
			else {
				...
				if (Info != NULL) {
					if (Info->eTxBf || Info->iTxBf) {
						tmac_info->AntPri = 0;
						/* MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("DBDC AntPri = 0 for TxBF\n")); */
					}
				}
			}
#endif /* DBDC_MODE */
	...
	if ((pkt_tx_time > 0) && (need_ampdu)) {
#ifdef ATE_CONCURRENT_SUPPORT
		tmac_info->Wcid = (UINT8)TESTMODE_GET_PARAM(ATECtrl, band_idx, wcid_ref);
#else
...
INT mt_ate_wtbl_cfg_v2(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	ldpc = TESTMODE_GET_PARAM(ATECtrl, band_idx, Ldpc);
	ant_sel = TESTMODE_GET_PARAM(ATECtrl, band_idx, TxAntennaSel);
	preamble = TESTMODE_GET_PARAM(ATECtrl, band_idx, Preamble);
...
INT32 MT_ATETxPkt(RTMP_ADAPTER *pAd, UINT32 band_idx)
{
	...
	if (pkt_tx_time > 0) {
#ifdef ATE_CONCURRENT_SUPPORT
		UINT8 wcid = TESTMODE_GET_PARAM(ATECtrl, band_idx, wcid_ref);
#else
...
#if defined(MT7663) || defined(MT7626)  /* tmp version for DE Debug */
#else
...
static INT32 MT_ATEStartTx(RTMP_ADAPTER *pAd)
{
...
#ifdef ATE_CONCURRENT_SUPPORT
	UINT32 wcid = TESTMODE_GET_PARAM(ATECtrl, control_band_idx, wcid_ref);
#endif
#ifdef ARBITRARY_CCK_OFDM_TX
	...
	if (IS_MT7663(pAd) || IS_MT7626(pAd)) {
		MtATETestModeTxControl(pAd, control_band_idx, TRUE);
	}
#endif
#endif

	if (Rate == 32)
#ifdef ATE_CONCURRENT_SUPPORT /* remove */
		mt_ate_mcs32_handle(pAd, wcid, bw);
#else
		mt_ate_mcs32_handle(pAd, ATECtrl->wcid_ref, bw);
#endif

#if defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT)
	/* dont stop rx at mac layer to avoid another band cannot do rx  */
#else
...
#if defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT)
	MT_ATEUpdateConcurrentStatus(pAd, control_band_idx, ATE_TXFRAME);
#endif
...
#if defined(DBDC_MODE) && defined(ATE_CONCURRENT_SUPPORT)
	MT_ATEUpdateConcurrentStatus(pAd, control_band_idx, ATE_RXFRAME);
#endif
...
#endif
	UCHAR control_band_idx = ATECtrl->control_band_idx;
	UINT32 Band = (control_band_idx ==  0) ? 24070000 : 50000000;
	UINT32 Mode = TESTMODE_GET_PARAM(ATECtrl, control_band_idx, Mode);
...
#ifdef ATE_CONCURRENT_SUPPORT
				ops->sta_clean_queue(pAd, wcid);
#else
				ops->sta_clean_queue(pAd, WCID_ALL);
#endif
			}
#if defined(MT7663)
			/* Wait Pdma is empty, AgeTime :(10 sec) */
			MtATETestModePdmaWaitIdle(pAd, HIF_TX_IDX1, 100, 100);
#endif
...
#if defined(MT7663) || defined(MT7626)
		if (IS_MT7663(pAd) || IS_MT7626(pAd)) {
			MtATETestModeTxControl(pAd, control_band_idx, FALSE);
		}
#endif
#endif
...
static INT32 MT_ATESetTxAntenna(RTMP_ADAPTER *pAd, UINT32 Ant)
{
	...
	for (ant_loop = 0; ant_loop < nss_cap->max_nss; ant_loop++)
...
static INT32 MT_ATESetTxFreqOffset(RTMP_ADAPTER *pAd, UINT32 FreqOffset)
{
	...
	UCHAR control_band_idx = ATECtrl->control_band_idx;
	INT32 Ret = 0;
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, RFFreqOffset, FreqOffset);
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s\n", __func__));
#ifdef CONFIG_HW_HAL_OFFLOAD
					...
					UINT32 Ch_Band)
{
	...
	pWdev = pAd->wdev_list[wdev_idx];
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: wdev_idx=%d, ch=%d, Ch_Band=%d\n",
		__func__, wdev_idx, ch, Ch_Band));
	...
	if (IS_ATE_DBDC(pAd))
		max_stream_num = cap->mcs_nss.max_nss / 2 ? cap->mcs_nss.max_nss / 2 : 1;
	else
	...
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, rx_strm_pth, rx_sel);
	TESTMODE_SET_PARAM(ATECtrl, control_band_idx, tx_strm_num, tx_stream_num);
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
...
#if !defined(COMPOS_TESTMODE_WIN)
	/* QA mode used central ch, thus wdev ch set as qa mode's control ch of relating BW setting */
	pWdev->channel = ch_cfg.ControlChannel;
	...
	mt7663_apply_dpd_flatness_data(pAd, ch_cfg);
#endif
#endif
...
#ifdef SINGLE_SKU_V2
	MtPwrLimitTblChProc(pAd, ch_cfg.BandIdx, ch_cfg.Channel_Band, ch_cfg.ControlChannel, ch_cfg.CentralChannel);
#endif
...
static INT32 MT_ATEGetDBDCTxTonePower(RTMP_ADAPTER *pAd, PINT32 pPwr, UINT32 AntIdx)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s: AntIdx:%d, control_band_idx:%d\n",
		__func__, AntIdx, control_band_idx));
#if defined(CONNAC_TXTONE_POWER_OFFSET)
...
static INT32 MT_ATEGetTxPower(RTMP_ADAPTER *pAd, UINT32 Channel, UINT32 Ch_Band, UINT32 u4AntIdx, PUINT32 Power)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s: Channel: %d, u1BandIdx: %d, u4AntIdx: %d\n",
		__func__, Channel, u1BandIdx, u4AntIdx));
...
INT32 MT_ATERFTestCB(RTMP_ADAPTER *pAd, UINT8 *Data, UINT32 Length)
{
...
#if defined(PRE_CAL_MT7622_SUPPORT) || defined(PRE_CAL_MT7663_SUPPORT) || defined(PRE_CAL_MT7626_SUPPORT)
	...
	case RE_CALIBRATION:
		if (data) {
			...
			if (IS_MT7622(pAd) || IS_MT7663(pAd) || IS_MT7626(pAd)) {
				if (pAd->bPreCalMode)
					os_alloc_mem(pAd, (UCHAR **)&cal_log, CAL_LOG_SIZE);
			}
#endif /* PRE_CAL_MT7622_SUPPORT || PRE_CAL_MT7663_SUPPORT || PRE_CAL_MT7626_SUPPORT */
			...
			if ((cal_type == PRE_CAL) || (cal_type == TX_DPD_FLATNESS_CAL)) {
				for (i = 0; i < len; i++) {
					...
					MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("[Recal][%08x][%08x]\n", cal_type, dump_tmp[i]));
...
static INT32 MT_MPSSetParm(RTMP_ADAPTER *pAd, enum _MPS_PARAM_TYPE type, INT32 items, UINT32 *data)
{
	...
	mps_setting = mps_cb->mps_setting;
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s: control_band_idx:%u, items:%d, Mode:%x, mps_cb:%p, mps_set:%p, mps_cnt:%d\n",
		__func__, control_band_idx, items, mode, mps_cb, mps_setting, mps_cb->mps_cnt));
...
static INT32 MT_ATESetAutoResp(RTMP_ADAPTER *pAd, UCHAR *mac, UCHAR mode)
{
	...
	if (control_band_idx == TESTMODE_BAND1) {
		...
		MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s OwnMacIdx:%d\n", __func__, OwnMacIdx));
	}
#endif
...
#endif
	UINT32 Rate = TESTMODE_GET_PARAM(ATECtrl, control_band_idx, Mcs);
#if defined(ATE_CONCURRENT_SUPPORT) /* remove */
	UINT8 wcid_ref = TESTMODE_GET_PARAM(ATECtrl, control_band_idx, wcid_ref);
#endif
#ifdef CONFIG_AP_SUPPORT
	INT32 IdBss, MaxNumBss = pAd->ApCfg.BssidNum;
#endif
...
static UINT32 MT_TestModeOpInit(RTMP_ADAPTER *pAd)
{
...
#endif /*PRE_CAL_MT7622_SUPPORT*/
#ifdef PRE_CAL_MT7663_SUPPORT
	AteOp->PreCal7663 = MtATE_Pre_Cal_Store_Proc_7663;
	AteOp->TxDPD7663  = MtATE_DPD_Cal_Store_Proc_7663;
	...
	AteOp->GetDumpRXV = MT_ATEGetDumpRXV;
...
#ifdef PRE_CAL_MT7663_SUPPORT
INT MtATE_Pre_Cal_Store_Proc_7663(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	USHORT                doCal1 = 0;
...
INT MtATE_DPD_Cal_Store_Proc_7663(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	UINT8                 i = 0;
	ULONG                 x = simple_strtol(arg, 0, 10);
	USHORT                doCal1 = 0;
    ...
    if (x == 0) {
		...
		for (i = 0; i < MT7663_DPD_FLATNESS_ABAND_BW20_CH_SIZE; i++) {
			/* set channel command , per group calibration - set to channel 36, 52, BW20 */
			ch_cfg.Bw = BW_20;
			ch_cfg.CentralChannel = MT7663_DPD_FLATNESS_ABAND_BW20_CH[i];
			ch_cfg.TxStream = 2;
			ch_cfg.RxStream = 3;
			ch_cfg.ControlChannel = MT7663_DPD_FLATNESS_ABAND_BW20_CH[i];
			ch_cfg.ControlChannel2 = 0;
			ch_cfg.BandIdx = 0;
			ch_cfg.bScan = 0;
		...
		for (i = 0; i < MT7663_DPD_FLATNESS_GBAND_BW20_CH_SIZE; i++) {
			/* set channel command , per group calibration - set to channel 1, 6, 11, BW20 */
			ch_cfg.Bw = BW_20;
			ch_cfg.CentralChannel = MT7663_DPD_FLATNESS_GBAND_BW20_CH[i];
			ch_cfg.TxStream = 2;
			ch_cfg.RxStream = 3;
			ch_cfg.ControlChannel = MT7663_DPD_FLATNESS_GBAND_BW20_CH[i];
			ch_cfg.ControlChannel2 = 0;
			ch_cfg.BandIdx = 0;
			ch_cfg.bScan = 0;
			MtCmdChannelSwitch(pAd, ch_cfg);
