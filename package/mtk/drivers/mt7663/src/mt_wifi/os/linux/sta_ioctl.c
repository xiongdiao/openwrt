...
struct iw_priv_args privtab[] = {
	...
	{
		RTPRIV_IOCTL_STA_SHOW, /* snowpin for ap/sta */
		IW_PRIV_TYPE_CHAR | 1024, 0,
		"show"
	},

	{
		RTPRIV_IOCTL_SUBCMD_SHOW, IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK,
		""
	},
	...
	{
		SHOW_WTBL_INFO_TLV,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "wtbl_tlv"
	},
#endif
	{
		SHOW_MIB_INFO,
	...
	{
		SHOW_PLE_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "pleinfo"
	},
	{
		SHOW_TXD_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "txdinfo"
	},
	{
		DUMP_MEM_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "dumpmem"
	},
	{
		SHOW_PSE_DATA,
	...
	{
		SHOW_CT_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "ctinfo"
	},
#endif /* CUT_THROUGH */
#ifdef DBDC_MODE
	{
		SHOW_DBDC_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "dbdcinfo"
	},
#endif /*DBDC_MODE*/
	{
		SHOW_TXOP_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "txopinfo"
	},
#endif /* MT_MAC */
#ifdef MEM_ALLOC_INFO_SUPPORT
	{
		SHOW_MEM_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "meminfo"
	},
	{
		SHOW_PKTMEM_INFO,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "pktmeminfo"
	},
#endif /* MEM_ALLOC_INFO_SUPPORT */
	{
		SHOW_WIFI_INT_CNT,
		IW_PRIV_TYPE_CHAR | 1024, IW_PRIV_TYPE_CHAR | IW_PRIV_SIZE_MASK, "wfintcnt"
	},
	{
		SHOW_EFUSE_INFO,
...
INT DetermineCallerInterface(struct net_device *net_dev, VOID *pAd)
{
	RT_CMD_IOCTL_INTERFACE_CONFIG   IoctlConfig, *pIoctlConfig = &IoctlConfig;
	/* Determine the interface idx */
	...
	pIoctlConfig->interface_idx = 0;

	if (RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_PREPARE, 0,
							 pIoctlConfig, 0, RT_DEV_PRIV_FLAGS_GET(net_dev)) != NDIS_STATUS_SUCCESS) {
		/* prepare error */
		return -EFAULT;
	}
					 ...
					 __u32 *mode, char *extra)
{
	...
	if (*mode == IW_MODE_ADHOC)
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>rt_ioctl_siwmode::SIOCSIWMODE (unknown %d)\n", *mode));
		return -EINVAL;
	}
...
#define PRISM_TYPE 1
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>rt_ioctl_giwmode(mode=%d)\n", *mode));
	return 0;
}
					  ...
					  struct iw_point *data, char *extra)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>rt_ioctl_giwrange\n"));
	...
	for (i = 1; i <= range->num_channels && i < IW_MAX_FREQUENCIES; i++) {
		/*		UINT32 m = 2412000; */
	...
	/* what is correct max? This was not documented exactly. At least 69 has been observed. */
	...
	struct iw_point *dwrq,	char *extra)
{
	...
	if (copy_from_user(ext, dwrq->pointer, len)) {
		kfree(ext);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("andriod_handle_private   copy_from_user\n"));
		return -EFAULT;
	}
	...
	if (rtstrcasecmp(ext, "START") == TRUE) {
		...
		/* OK if successful */
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("sSTART Turn on Wi-Fi hardware\n"));
		kfree(ext);
		return -1;
	} else if (rtstrcasecmp(ext, "STOP") == TRUE) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("STOP Turn off  Wi-Fi hardware\n"));
		kfree(ext);
		return -1;
				   ...
				   struct sockaddr *ap_addr, char *extra)
{
	...
	if (RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_STA_SIOCGIWAP, 0,
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IOCTL::SIOCGIWAP(=EMPTY)\n"));
		return -ENOTCONN;
	}
					  ...
					  struct iw_point *data, char *extra)
{
	...
	for (i = 0; i < IW_MAX_AP; i++) {
		...
		memcpy(extra + i * sizeof(addr[i]), &addr[i], sizeof(addr[i]));
					 ...
					 struct iw_point *data, char *extra)
{
	...
	for (i = 0; i < pIoctlScan->BssNr; i++) {
		...
		previous_ev = current_ev;
		Index = (iwe.u.data.flags & IW_ENCODE_INDEX) - 1;
		...
		current_ev = IWE_STREAM_ADD_POINT(info, current_ev, end_buf, &iwe, (char *)pIoctlScan->MainSharedKey[Index]);
		...
		/*================================ */
		if (pIoctlScan->pBssTable[i].WpaIeLen > 0) {
			os_zero_mem(&iwe, sizeof(iwe));
			memset(&custom[0], 0, MAX_CUSTOM_LEN);
			iwe.cmd = IWEVCUSTOM;
			iwe.u.data.length = (pIoctlScan->pBssTable[i].WpaIeLen * 2) + 7;
			os_move_mem(custom, "wpa_ie=", 7);
	...
	/*    pStaCfg->bScanReqIsFromWebUI = FALSE; */
	/*	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR ,("===>rt_ioctl_giwscan. %d(%d) BSS returned, data->length = %d\n",i , pAd->ScanTab.BssNr, data->length)); */
					  ...
					  void *w, char *extra)
{
	...
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, this_char) != NDIS_STATUS_SUCCESS) {
		...
		if (value && !*value && (strcmp(this_char, "SSID") != 0))
			return -EINVAL;
	}
...
int rt_ioctl_showparam(struct net_device *dev, struct iw_request_info *info,
					   void *w, char *extra)
{
	VOID *pAd;
	/*	POS_COOKIE pObj; */
	...
	RTMP_STA_IoctlHandle(pAd, NULL, CMD_RTPRIV_IOCTL_PARAM_SHOW, 0,
						 &CmdParam, 0, RT_DEV_PRIV_FLAGS_GET(dev));
	return Status;
}
...
#ifdef WCX_SUPPORT
static int mt_private_meta_set(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	INT	stat = 0;
	VOID *pAd = NULL;

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<== %s(), wrq->length = %d\n", __func__, wrq->length));
	GET_PAD_FROM_NET_DEV(pAd, dev);
	RTMP_STA_IoctlHandle(pAd, wrq, CMD_MTPRIV_IOCTL_STA_META_SET, 0, extra, IW_PRIV_SIZE_MASK, RT_DEV_PRIV_FLAGS_GET(dev));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s()==>\n", __func__));
	return stat;
}

static int mt_private_meta_qurey(struct net_device *dev, struct iw_request_info *info, struct iw_point *wrq, char *extra)
{
	INT	stat = 0;
	VOID *pAd = NULL;
	RTMP_IOCTL_INPUT_STRUCT wrqin;

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<== %s(), wrq->length = %d, wrq->pointer:%p\n", __func__, wrq->length, wrq->pointer));
	GET_PAD_FROM_NET_DEV(pAd, dev);
	wrqin.u.data.pointer = wrq->pointer;
	wrqin.u.data.length = wrq->length;
	RTMP_STA_IoctlHandle(pAd, &wrqin, CMD_MTPRIV_IOCTL_STA_META_QUERY, 0, extra, IW_PRIV_SIZE_MASK, RT_DEV_PRIV_FLAGS_GET(dev));
						  ...
						  struct iw_point *wrq, char *extra)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<== rt_private_get_statistics, wrq->length = %d\n", wrq->length));
	return Status;
}
					 ...
					 char *extra)
{
	...
	default:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("====> %s - Unknow Command\n", __func__));
		break;
	}
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	case IW_AUTH_WPA_ENABLED:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_WPA_ENABLED - Driver supports WPA!(param->value = %d)\n", __func__, param->value));
		break;
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_giwauth::param->value = %d!\n", param->value));
	return 0;
}
						  ...
						  char *extra)
{
	...
	if (alg == IW_ENCODE_ALG_NONE)
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_WARN, ("Warning: Security type is not supported. (alg = %d)\n", alg));
		pIoctlSec->Alg = alg;
		return -EOPNOTSUPP;
	}
					  ...
					  union iwreq_data *wrqu, char *extra)
{
	...
	GET_PAD_FROM_NET_DEV(pAd, dev);
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> rt_ioctl_giwencodeext\n"));
					  ...
					  char *extra)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===> rt_ioctl_siwpmksa\n"));
					 ...
					 struct iw_point *wrq, char *extra)
{
	...
	RTMP_STA_IoctlHandle(pAd, &wrqin, CMD_RTPRIV_IOCTL_BBP, 0,
						 extra, IW_PRIV_SIZE_MASK, RT_DEV_PRIV_FLAGS_GET(dev));
	wrq->length = wrqin.u.data.length;
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("<==rt_private_ioctl_bbp\n\n"));
	return Status;
}
#endif /* DBG */
					 ...
					 union iwreq_data *wrqu, char *extra)
{
	...
	/*	if(!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_INTERRUPT_IN_USE)) */
	if (RTMP_DRIVER_IOCTL_SANITY_CHECK(pAd, NULL) != NDIS_STATUS_SUCCESS) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwrate::Network is down!\n"));
		return -ENETDOWN;
	}

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwrate::(rate = %d, fixed = %d)\n", rate, fixed));
	/* rate = -1 => auto rate */
...
static const iw_handler rt_priv_handlers[] = {
...
#ifdef WCX_SUPPORT
	(iw_handler) mt_private_meta_set, /* + 0x08 */
	(iw_handler) mt_private_meta_qurey, /* + 0x09 */
#else
...
#ifdef WCX_SUPPORT
	(iw_handler) rt_private_get_statistics, /* + 0x0B */
#else
	...
	(iw_handler) rt_ioctl_showparam, /* + 0x12 */
...
#endif /* WSC_STA_SUPPORT */
#ifdef WCX_SUPPORT
	(iw_handler) rt_private_get_statistics, /* + 0x15 */
#else
...
INT rt28xx_sta_ioctl(void *net_dev_obj, void *rq, INT cmd) /* snowpin for ap/sta */
{
	/* POS_COOKIE pObj; */
	...
	if (wrqin == NULL) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("INFO::wrqin is null!\n"));
		return Status;
	}
	...
	if (DetermineCallerInterface(net_dev, pAd) != NDIS_STATUS_SUCCESS)
		return -EFAULT;
	...
	case SIOCGIFHWADDR:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IOCTL::SIOCGIFHWADDR\n"));
	...
	case RT_PRIV_IOCTL:
			...
			rt_android_private_command_entry(pAd, net_dev, (struct ifreq *)rq, cmd);
		break;
	...
	case SIOCGIWPRIV:
		if (wrqin->u.data.pointer) {
			...
			if ((ARRAY_SIZE(privtab)) <= wrq->u.data.length) {
				wrqin->u.data.length = ARRAY_SIZE(privtab);
