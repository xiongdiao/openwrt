	...
	IN PNDIS_PACKET	pPacket)
{
	...
	if (ops->rx_announce_802_3_pkt) {
		if (!ops->rx_announce_802_3_pkt(pAd, wdev, pRxPkt)) {
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s(): rx_announce_802_3_pkt drop this paket!\n", __func__));
				return;
		}
	}
	...
	if (ops->rx_looping_detect) {
		if (!ops->rx_looping_detect(pAd, wdev, pRxPkt)) {
				MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					("%s(): rx_looping_detect drop this paket!\n", __func__));
				return;
		}
	}
