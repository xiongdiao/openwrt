...
VOID dump_vht_op(RTMP_ADAPTER *pAd, VHT_OP_IE *vht_ie)
{
	...
	MTWF_LOG(DBG_CAT_PROTO, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("\tChannelCenterFrequency Seg 0=%d\n", vht_op->ccfs_0));
...
INT build_vht_oper_ie(RTMP_ADAPTER *pAd, UCHAR bw, UCHAR Channel, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	UCHAR cen_ch_2;
	struct _RTMP_CHIP_CAP *cap = hc_get_chip_cap(pAd->hdev_ctrl);
	UINT8 max_vht_mcs = cap->mcs_nss.max_vht_mcs;
...
INT build_vht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR *buf)
{
	...
	vht_cap_ie.vht_cap.max_mpdu_len = cap->ppdu.max_mpdu_len;
	...
	if (pAd->CommonCfg.vht_mcs_cap < cap->mcs_nss.max_vht_mcs)
...
static inline UINT32 set_ch_width_ext_nss_bw_caps(struct wifi_dev *wdev, UINT8 ch_width)
{
	...
	if (ch_width > VHT_BW_80) {
		if (nss_cap->max_nss == nss_cap->bw160_max_nss) {
			/*support_ch_width_set = 2, ext_nss_bw = 0*/
			cap_info |= (CH_WIDTH_SET_2 << IEEE80211_VHT_CAP_SUPPORT_CH_WIDTH_SET_SHIFT);
			wlan_config_set_vht_ext_nss_bw(wdev, 0);
		}
		if ((nss_cap->max_nss / 2) == nss_cap->bw160_max_nss) {
			/*support_ch_width_set = 0, ext_nss_bw = 2*/
			cap_info |= (EXT_NSS_BW_2 << IIEE80211_VHT_CAP_EXT_NSS_BW_SHIFT);
			wlan_config_set_vht_ext_nss_bw(wdev, 2);
		}
		if ((nss_cap->max_nss << 1) == nss_cap->bw160_max_nss) {
			/*support_ch_width_set = 2, ext_nss_bw = 3*/
			cap_info |= ((CH_WIDTH_SET_2 << IEEE80211_VHT_CAP_SUPPORT_CH_WIDTH_SET_SHIFT)
					| (EXT_NSS_BW_2 << IIEE80211_VHT_CAP_EXT_NSS_BW_SHIFT));
...
static inline UINT16 set_vht_mcs_map(UINT8 stream, UINT8 max_mcs)
{
	...
	mcs_map &= ~(IEEE80211_VHT_MCS_1SS_MASK);
	mcs_map |= VHT_MAX_MCS_NSS(1, max_mcs);
	if (stream > 1) {
		mcs_map &= ~(IEEE80211_VHT_MCS_2SS_MASK);
		mcs_map |= VHT_MAX_MCS_NSS(2, max_mcs);
	}
	if (stream > 2) {
		mcs_map &= ~(IEEE80211_VHT_MCS_3SS_MASK);
		mcs_map |= VHT_MAX_MCS_NSS(3, max_mcs);
	}
	if (stream > 3) {
		mcs_map &= ~(IEEE80211_VHT_MCS_4SS_MASK);
		mcs_map |= VHT_MAX_MCS_NSS(4, max_mcs);
	}
	if (stream > 4) {
		mcs_map &= ~(IEEE80211_VHT_MCS_5SS_MASK);
		mcs_map |= VHT_MAX_MCS_NSS(5, max_mcs);
	}
	if (stream > 5) {
		mcs_map &= ~(IEEE80211_VHT_MCS_6SS_MASK);
		mcs_map |= VHT_MAX_MCS_NSS(6, max_mcs);
	}
	if (stream > 6) {
		mcs_map &= ~(IEEE80211_VHT_MCS_7SS_MASK);
		mcs_map |= VHT_MAX_MCS_NSS(7, max_mcs);
	}
	if (stream > 7) {
		mcs_map &= ~(IEEE80211_VHT_MCS_8SS_MASK);
...
static UINT8 *build_vht_caps_info(struct wifi_dev *wdev, UINT8 *f_buf)
{
	...
	/* max_mpdu_length */
	cap_info |= (mpdu->max_mpdu_len & IEEE80211_VHT_CAP_MAX_MPDU_LEN_MASK);
	/* support_ch_width_set */
	cap_info |= set_ch_width_ext_nss_bw_caps(wdev, support_ch_width);
	/* rx_ldpc */
	if (rx_ldpc)
		cap_info |= IEEE80211_VHT_CAP_RX_LDPC;
	/* bw80/bw160_8080 sgi */
	if (sgi) {
		cap_info |= IEEE80211_VHT_CAP_BW80_SGI;
		if (support_ch_width > VHT_BW_80)
			cap_info |= IEEE80211_VHT_CAP_BW160_8080_SGI;
	}
	/* tx/rx stbc */
	if (stbc)
		cap_info |= (IEEE80211_VHT_CAP_TX_STBC | (1 << IEEE80211_VHT_CAP_RX_STBC_SHIFT));
	/* bf */
	cap_info |= set_vht_bf_caps(wdev);
	/* max ampdu length exp */
	cap_info |= (mpdu->vht_max_ampdu_len_exp << IEEE80211_VHT_CAP_MAX_AMPDU_LEN_EXP_SHIFT);
...
static UINT8 *build_vht_mcs_nss_set(struct wifi_dev *wdev, UINT8 *f_buf)
{
	...
	tx_rate_ext_nss_bw |= IEEE80211_VHT_MCS_EXT_NSS_BW_CAPABLE;
...
UINT8 *build_vht_op_info(struct wifi_dev *wdev,
		UINT8 prim_ch, UINT8 bw, UINT8 nss_diff, UINT8 *f_buf)
{
