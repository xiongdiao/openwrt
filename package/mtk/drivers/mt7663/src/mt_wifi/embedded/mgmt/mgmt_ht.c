...
UINT32 starec_ht_feature_decision(struct wifi_dev *wdev, struct _MAC_TABLE_ENTRY *entry, UINT32 *feature)
{
	...
	if ((cap->ppdu.SupportAMSDU == TRUE) && amsdu_en
		/*
	...
	IN ADD_HT_INFO_IE * pAddHtInfo)
{
	...
	ASSERT(pStaCfg);
	if (pStaCfg == NULL) {
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s StaCfg NULL!! wcid %d entryType %d\n", __func__, Wcid, sta->EntryType));
		if (sta->wdev)
			MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("wdev %s, idx %d\n", sta->wdev->if_dev->name, sta->wdev->wdev_idx));
		return FALSE;
	}
...
*/
		...
		if ((pStaCfg->MlmeAux.HtCapability.HtCapInfo.ChannelWidth == BW_40) && (wlan_operate_get_ext_cha(sta->wdev) != pAddHtInfo->AddHtInfo.ExtChanOffset)) {
			...
			if ((extcha != pAddHtInfo->AddHtInfo.ExtChanOffset) && (sta->wdev->channel > 14)) {
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR, ("RTMPCheckHt :  channel=%u,	my extcha=%u, root ap extcha=%u, inconsistent!!\n",
...
static INT build_ht_cap_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR ReptMacTabWCID, UCHAR *buf)
{
	UCHAR HtLen = 0;
	INT len = 0;
	HT_CAPABILITY_IE HtCapabilityTmp;
	...
	if (wdev && (wdev->wdev_type == WDEV_TYPE_STA) && pStaCfg
		...
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				("%s : Setting Green Field for Ap-Cli\n", __func__));
	} else {
		if (!pStaCfg)
			...
			if ((ReptMacTabWCID != 0) && (pAd->fgClonedStaWithBfeeSelected == FALSE)) {
				...
				if (pEntry == NULL) {
					MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
							 ("%s : HT This entry isn't belong to cloned STA!!!===============\n",
							  __func__));
				}
				...
				if ((pEntry) && (HcIsBfCapSupport(pEntry->wdev) == TRUE)) {
					...
					if (WMODE_HT_ONLY(wdev->PhyMode)) {
						...
						pAd->ReptClonedStaEntry_CliIdx	 = pEntry->MatchReptCliIdx;
					}
				}
			}
...
static INT build_bss_coex_ie(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, UCHAR Channel, UCHAR *buf)
{
	INT len = 0;
#ifdef DOT11N_DRAFT3
	if ((Channel <= 14) &&
		(pAd->CommonCfg.bBssCoexEnable == TRUE)) {
		UCHAR BssCoexistIeLen = 1;

		MAKE_IE_TO_BUF(buf, &BssCoexistIe, 1, len);
		MAKE_IE_TO_BUF(buf, &BssCoexistIeLen, 1, len);
		MAKE_IE_TO_BUF(buf, &pAd->CommonCfg.BSSCoexist2040.word, 1, len);
	}
#endif /* DOT11N_DRAFT3 */
...
INT build_ht_ies(RTMP_ADAPTER *pAd, struct _build_ie_info *info)
{
	...
	len += build_bss_coex_ie(pAd, info->wdev, info->channel, (UCHAR *)(info->frame_buf + len));
