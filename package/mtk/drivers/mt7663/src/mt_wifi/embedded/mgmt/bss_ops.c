...
VOID BssTableInit(BSS_TABLE *Tab)
{
	...
	for (i = 0; i < MAX_LEN_OF_BSS_TABLE; i++) {
		...
		if (Tab->BssEntry[i].CustomerBssEntry.vendor_ie.pointer != NULL) {
			os_free_mem(Tab->BssEntry[i].CustomerBssEntry.vendor_ie.pointer);
			Tab->BssEntry[i].CustomerBssEntry.vendor_ie.pointer = NULL;
		}
		Tab->BssEntry[i].CustomerBssEntry.vendor_ie.length = 0;
...
static VOID update_bss_by_owe_trans(struct _RTMP_ADAPTER *ad,
				    ...
				    UCHAR pair_ssid_len)
{
	BSS_ENTRY *extracted_trans_bss = &ad->ScanCtrl[band_idx].ScanTab.BssEntry[bss_idx];
			...
			if ((vht_op->vht_op_info.ch_width > 0) &&
				...
				MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						 ("%s():VHT vht_op_info->center_freq_1=%d, Bss->CentCh=%d, change from CentralChannel to cent_ch!\n",
						  __func__, vht_op->vht_op_info.ccfs_0, pBss->CentralChannel));
...
#ifdef CUSTOMER_VENDOR_IE_SUPPORT
	CustomerBssEntrySet(pAd, ie_list, pBss, LengthVIE, pVIE);
#endif /* CUSTOMER_VENDOR_IE_SUPPORT */
}
	...
	IN PNDIS_802_11_VARIABLE_IEs pVIE)
{
	...
	if (ie_list->Channel > 14) {
		if (!ie_list->HtCapabilityLen && !ie_list->vht_cap_len) {
...
*/
	...
	if (ie_list->HtCapabilityLen) {
		MODE = MODE_HTMIX;
		if (ie_list->HtCapability.HtCapInfo.ShortGIfor20 == TRUE ||
		    ie_list->HtCapability.HtCapInfo.ShortGIfor40 == TRUE)
			ShortGI = 1;

		BW = CustomerBssEntry->ht_ch_bandwidth;

		if ((ie_list->HtCapability.MCSSet[3] & 0xff) == 0xff) {
			MAX_MCS = 31;
			CustomerBssEntry->ht_tx_ss = 4;
			CustomerBssEntry->ht_rx_ss = 4;
		} else if ((ie_list->HtCapability.MCSSet[2] & 0xff) == 0xff) {
			MAX_MCS = 23;
			CustomerBssEntry->ht_tx_ss = 3;
			CustomerBssEntry->ht_rx_ss = 3;
		} else if ((ie_list->HtCapability.MCSSet[1] & 0xff) == 0xff) {
			MAX_MCS = 15;
			CustomerBssEntry->ht_tx_ss = 2;
			CustomerBssEntry->ht_rx_ss = 2;
		} else if ((ie_list->HtCapability.MCSSet[0] & 0xff) == 0xff) {
			MAX_MCS = 7;
...
#ifdef DOT11_VHT_AC
	if (ie_list->vht_cap_len > 0 && ie_list->Channel > 14) {
		UCHAR mcs_ss = 9;

		MODE = MODE_VHT;
		if (ie_list->vht_cap_ie.vht_cap.sgi_80M == TRUE ||
		    ie_list->vht_cap_ie.vht_cap.sgi_160M == TRUE)
			ShortGI = 1;

		BW = CustomerBssEntry->vht_ch_bandwidth;

		if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss8 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 8;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss8;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss7 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 7;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss7;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss6 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 6;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss6;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss5 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 5;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss5;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss4 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 4;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss4;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss3 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 3;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss3;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss2 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 2;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss2;
		} else if (ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss1 != VHT_MCS_CAP_NA) {
			CustomerBssEntry->vht_tx_ss = 1;
			mcs_ss = ie_list->vht_cap_ie.mcs_set.tx_mcs_map.mcs_ss1;
		}

		if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss8 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 8;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss7 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 7;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss6 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 6;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss5 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 5;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss4 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 4;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss3 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 3;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss2 != VHT_MCS_CAP_NA)
			CustomerBssEntry->vht_rx_ss = 2;
		else if (ie_list->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss1 != VHT_MCS_CAP_NA)
	...
	CustomerBssEntry->vendor_ie.pointer = ie_list->CustomerVendorIE.pointer;
	ie_list->CustomerVendorIE.pointer = NULL;
	...
	IN UCHAR SsidLen)
{
	...
	for (i = 0; i < ScanTab->BssNr && ScanTab->BssNr < MAX_LEN_OF_BSS_TABLE; i++) {
		...
		if (pInBss->hide_open_owe_bss) {
			MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("%s : skip %02x-%02x-%02x-%02x-%02x-%02x, SSID:%s, AKM:0x%x by OWE transition\n",
				__func__,
				PRINT_MAC(pInBss->Bssid),
				pInBss->Ssid,
				pInBss->AKMMap));
			continue;
		}
       ...
       && (pStaCfg->MlmeAux.Channel > 14)
		...
		if ((IS_AKM_WPA3_192BIT(pSecConfig->AKMMap) || IS_AKM_WPA3PSK(pSecConfig->AKMMap) || IS_AKM_WPA3(pSecConfig->AKMMap))
			...
			if (RsnCap.field.MFPC == FALSE) {
				MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_INFO, ("[PMF]%s : Peer's MPFC isn't used.\n", __func__));
				continue;
			}
			...
			if (((pSecConfig->PmfCfg.MFPC == FALSE) && (RsnCap.field.MFPC == FALSE))
				...
				if ((pSecConfig->PmfCfg.PMFSHA256) && (pInBss->IsSupportSHA256KeyDerivation == FALSE)) {
					MTWF_LOG(DBG_CAT_SEC, CATSEC_PMF, DBG_LVL_INFO, ("[PMF]%s : Peer is not sha256.\n", __func__));
					continue;
				}
			}
		}
      ...
      && (pInBss->BssType == pStaCfg->BssType)
			...
			MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					 ("-------->%s(%d), AKMMap=0x%x, PairwiseCipher=0x%x, GroupCipher=0x%x, CapabilityInfo=0x%x\n",
					  __func__, __LINE__, pInBss->AKMMap, pInBss->PairwiseCipher, pInBss->GroupCipher, pInBss->CapabilityInfo));
			OutTab->BssNr++;
		} else if ((SsidLen == 0)
      && (pInBss->BssType == pStaCfg->BssType)
      && OutTab->BssNr < MAX_LEN_OF_BSS_TABLE) {
			...
			MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					 ("-------->%s(%d), AKMMap=0x%x, PairwiseCipher=0x%x\n",
					  __func__, __LINE__, pInBss->AKMMap, pInBss->PairwiseCipher));
			OutTab->BssNr++;
		}
