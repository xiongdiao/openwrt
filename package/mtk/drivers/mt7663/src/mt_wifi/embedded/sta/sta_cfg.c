 ...
 *
 * (c) Copyright 2002-2006,2018 Ralink Technology, Inc.
 *
...
INT set_monitor_channel(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (arg) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():rv = %d, bw = %d, channel = %d, pri_idx = %d\n", __func__,
		...
		if (rv == 3) {
			if (bw == BW_40) {
				if (pri_idx == 0)
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("pri_idx(%d) is invalid\n", pri_idx));
					return FALSE;
				}
			}
				...
				vht_bw = VHT_BW_8080;
				cen_ch_2 = vht_cent_ch_freq(pri_idx, VHT_BW_8080);
			}
	...
	IN RTMP_STRING *ProcArg)
{
	...
	if (PRTMP_PRIVATE_SHOW_PROC->name == NULL) {
		/*Not found argument */
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
				 ("===>rt_ioctl_setparam:: (iwpriv) Not Support Show Command [%s=%s]\n", SetProcName, ProcArg));
		return -EINVAL;
	}
	...
	if (strlen(arg) <= MAX_LEN_OF_SSID) {
		...
		if (strlen(arg) != 0) {
...
#ifdef WCX_SUPPORT
			{
				...
				if (boot_mode == FACTORY_BOOT) {
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! arg:%s, CommonCfg.Ssid:%s, SsidLen:%d\n", Ssid.Ssid,
...
#ifdef DOT11W_PMF_SUPPORT
		{
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("[PMF]%s:: MFPC=%d, MFPR=%d, SHA256=%d\n",
					 __func__, pPmfCfg->MFPC, pPmfCfg->MFPR,
					 pPmfCfg->PMFSHA256));
		}
#endif /* DOT11W_PMF_SUPPORT */
		...
		if (Ssid.SsidLength == MAX_LEN_OF_SSID)
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_SSID_Proc::(Len=%d,Ssid=%s)\n", Ssid.SsidLength, Ssid.Ssid));
	} else
		success = FALSE;
...
INT	Set_WmmCapable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WmmCapable_Proc::(bWmmCapable=%d)\n",
...
INT	Set_APSDAC_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (i  !=  WMM_NUM_OF_AC) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Invalid parameters\n"));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("APSDAC::(BE,BK,VI,VO)=(%d,%d,%d,%d)\n",
...
INT Set_NetworkType_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (strcmp(arg, "Adhoc") == 0) {
		if (pStaCfg->BssType != BSS_ADHOC) {
			...
			if (INFRA_ON(pStaCfg)) {
				...
				NdisZeroMemory(pStaCfg->MlmeAux.AutoReconnectSsid, pStaCfg->MlmeAux.AutoReconnectSsidLen);
				LinkDown(pAd, FALSE, &pStaCfg->wdev, NULL);
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("NDIS_STATUS_MEDIA_DISCONNECT Event BB!\n"));
			}
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>Set_NetworkType_Proc::(AD-HOC)\n"));
		...
		pStaCfg->BssType = BSS_INFRA;
		RTMP_OS_NETDEV_SET_TYPE(pAd->net_dev, pStaCfg->OriDevType);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>Set_NetworkType_Proc::(INFRA)\n"));
	}
...
#endif /* MONITOR_FLAG_11N_SNIFFER_SUPPORT */
	{
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("fSTA_STATUS_MEDIA_STATE_CONNECTED\n"));
		wlan_operate_set_prim_ch(wdev, wdev->channel);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():BW_%s, CtrlChannel(%d), CentralChannel(%d)\n",
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>Set_NetworkType_Proc::(MONITOR)\n"));
#ifdef SNIFFER_MT7615
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_NetworkType_Proc::(NetworkType=%d)\n", pStaCfg->BssType));
	return TRUE;
}
...
INT Set_AuthMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: AKM=0x%x\n",
...
INT Set_EncrypType_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s:: CIPHER=0x%x\n",
			 __func__, pSecConfig->AKMMap));
	return TRUE;
}
	...
	IN	RTMP_STRING		*arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("IF(ra%d) Set_TdlsChannelSwitchBWProc::(TdlsChannelSwitchBW=%d)\n",
			  pObj->ioctl_if, pStaCfg->wpa_supplicant_info.CFG_Tdls_info.TargetOffChannelBW));
	return TRUE;
}
	...
	IN	RTMP_STRING		*arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("IF(ra%d) Set_CfgTdlsChannelSwitchBaseStayTimeProc::(BaseChannelStayTime=%d)\n",
			  pObj->ioctl_if, pStaCfg->wpa_supplicant_info.CFG_Tdls_info.BaseChannelStayTime));
	return TRUE;
}
...
INT Set_PSMode_Proc(RTMP_ADAPTER *pAdapter, RTMP_STRING *arg)
{
	...
	if (pStaCfg->BssType == BSS_INFRA) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_PSMode_Proc::(PSMode=%ld)\n", pStaCfg->WindowsPowerMode));
	} else
		return FALSE;
...
INT	 Set_WscConfMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra0) Set_WscConfMode_Proc::(WscConfMode(0,1,2)=%d)\n",
...
INT	Set_WscConfStatus_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if ((IsAPConfigured  > 0) && (IsAPConfigured  <= 2))
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WscConfStatus_Proc:: WscConfStatus is not changed (%d)\n",
				 pWscControl->WscConfStatus));
		return FALSE;  /*Invalid argument */
	}

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WscConfStatus_Proc::(WscConfStatus=%d)\n",
			 pWscControl->WscConfStatus));
	return TRUE;
}
...
INT	Set_WscMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if ((WscMode == WSC_SMPBC_MODE) &&
		(pStaCfg->BssType == BSS_INFRA)) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WscMode_Proc:: Set failed!!(Set_WscMode_Proc=%s)\n", arg));
		return FALSE;  /*Invalid argument */
	}
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WscMode_Proc::(WscMode=%d)\n",  pWscControl->WscMode));
	return TRUE;
}
...
INT	Set_WscPinCode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (validatePin) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WscPinCode_Proc:: Checksum is invalid\n"));
		return FALSE;
	}

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF(ra0) Set_WscPinCode_Proc::(PinCode=%d)\n",
...
INT	Set_WscUUIDE_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (UUIDLen != 32) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Length of UUID key is wrong length=%d\n", UUIDLen));
		return FALSE;
	}
	...
	for (i = 0; i < UUIDLen; i++) {
		if (!isxdigit(tmp_Uuid_Str[i])) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Input %d not Hex Value\n", i));
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The UUID Hex string is:"));

	for (i = 0; i < 16; i++)
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%02x", (pWscControl->Wsc_Uuid_E[i] & 0xff)));

	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\n"));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("The UUID ASCII string is:%s!\n", pWscControl->Wsc_Uuid_Str));
	return 0;
}
...
INT	Set_WscGetConf_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (INFRA_ON(pStaCfg)) {
		...
		pStaCfg->MlmeAux.CurrReqIsFromNdis = TRUE;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("disassociate with current AP before starting WPS\n"));
	...
	RTMPSetLED(pAd, WPSLEDStatus, HcGetBandByWdev((struct wifi_dev *)pWscControl->wdev));
#endif /* WSC_LED_SUPPORT */
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_WscGetConf_Proc trigger WSC state machine\n"));
	return TRUE;
}
...
INT Set_WscForceSetAP_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_WscForceSetAP_Proc::(bForceSetAP=%d)\n",
...
INT Set_TGnWifiTest_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_TGnWifiTest_Proc::(bTGnWifiTest=%d)\n",
...
INT Set_Ieee80211dClientMode_Proc(RTMP_ADAPTER *pAdapter, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set_Ieee802dMode_Proc::(IEEEE0211dMode=%d)\n",
...
INT Set_StaCarrierDetect_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_StaCarrierDetect_Proc::(CarrierDetect.Enable=%d)\n",
...
INT Set_BeaconLostTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_BeaconLostTime_Proc::(BeaconLostTime=%ld)\n",
...
INT Set_AutoRoaming_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_AutoRoaming_Proc::(bAutoRoaming=%d)\n",
...
INT Set_ForceTxBurst_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_ForceTxBurst_Proc::(bForceTxBurst=%d)\n",
...
INT Set_AutoReconnect_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_AutoReconnect_Proc::(bAutoReconnect=%d)\n",
...
INT Set_AdhocN_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("IF Set_AdhocN_Proc::(bAdhocN=%d)\n", adhocInfo->bAdhocN));
#endif /* DOT11_N_SUPPORT */
	return TRUE;
}
...
INT Set_SwitchIdleTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set_SwitchIdleTime_Proc::(switch_idle_time=%u)\n",
...
INT Set_NULL_Frame_Count_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Set_NULL_Frame_Count_Proc::(null_frame_count=%u)\n",
...
INT	Set_CH1_BW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("channel_1st_bw = %u\n", pStaCfg->channel_1st_bw));
	return TRUE;
}
...
INT	Set_CH2_BW_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("channel_2nd_bw = %u\n", pStaCfg->channel_2nd_bw));
	return TRUE;
}
...
INT	Set_CH1_Channel_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	 ...
	 *	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("channel_1st_channel = %u\n", pStaCfg->channel_1st_channel));
	 *	return TRUE;
	 */
}
...
INT	Set_CH2_Channel_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	 ...
	 *
	 *	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("channel_2nd_channel = %u\n", pStaCfg->channel_2nd_channel));
	 *	return TRUE;
	 */
}
...
INT Set_MCC_Start_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF Set_MCC_Start_Proc::(start=%d)\n", start));
	return TRUE;
}
...
INT Set_Pkt_Dbg_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("IF Set_Pkt_Dbg_Proc::(pStaCfg->bpkt_dbg=%d)\n",
			 pStaCfg->bpkt_dbg));
	return TRUE;
}
...
INT	Show_CH1StayTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("channel_1st_staytime = %u\n", pStaCfg->channel_2nd_staytime));
	return TRUE;
}

INT	Show_CH2StayTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("channel_2nd_staytime = %u\n", pStaCfg->channel_2nd_staytime));
	return TRUE;
}
...
INT	Show_SwitchIdleTime_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("switch_idle_time = %u\n", pStaCfg->switch_idle_time));
	return TRUE;
}
...
INT	Show_NULL_Frame_Count_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("null_frame_count = %u\n", pStaCfg->null_frame_count));
	return TRUE;
}
#endif /* CONFIG_MULTI_CHANNEL */
...
INT Set_WOW_Enable(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("WOW_Enable = %d\n", pAd->WOW_Cfg.bEnable));
		return TRUE;
	}
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WOW_Enable = %d, GPIO = %x\n", pAd->WOW_Cfg.bEnable, Val));
	return TRUE;
}
...
INT Set_WOW_GPIO(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("WOW_GPIO = %d\n", pAd->WOW_Cfg.nSelectedGPIO));
	return TRUE;
}
...
INT Set_WOW_Delay(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s] MTxxxx does not support this!\n", __func__));
		return FALSE;
	}
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WOW_Delay = %d, equal to %d sec\n", pAd->WOW_Cfg.nDelay,
...
INT Set_WOW_Hold(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("WOW_Hold = %d, equal to %d us\n", pAd->WOW_Cfg.nHoldTime,
...
INT Set_WOW_InBand(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("WOW_Inband = %d\n", pAd->WOW_Cfg.bInBand));
	return TRUE;
}
...
INT Set_WOW_Interface(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("WakeupInterface = %d\n", pAd->WOW_Cfg.nWakeupInterface));
		return TRUE;
	}
...
INT Set_WOW_IfDown_Support(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("WOW_IfDown_Support = %d\n", pAd->WOW_Cfg.bWowIfDownSupport));
		return TRUE;
	}
...
INT Set_WOW_IPAddress(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("IP Address = %d.%d.%d.%d\n",
...
INT Set_WOW_GPIOHighLow(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("GPIOHighLow = %d\n", pAd->WOW_Cfg.bGPIOHighLow));
		return TRUE;
	}
	...
	IN struct wifi_dev *wdev)
{
...
#endif /* DOT11_N_SUPPORT */
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-->RTMPSetInformation(),	0x%08x\n", cmd & 0x7FFF));
	...
	case RT_OID_802_11_COUNTRY_REGION:
		...
		/* Only avaliable when EEPROM not programming */
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_COUNTRY_REGION (A:%d  B/G:%d)\n",
	...
	case OID_802_11_BSSID_LIST_SCAN:
		...
		/*            Now = jiffies; */
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_BSSID_LIST_SCAN, TxCnt = %d\n",
		...
		if (pAd->RalinkCounters.LastOneSecTotalTxCount > 100) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! Link UP, ignore this set::OID_802_11_BSSID_LIST_SCAN\n"));
	...
	case OID_802_11_SET_PASSPHRASE:
		...
		if (ppassphrase == NULL) {
			Status = -ENOMEM;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_SET_PASSPHRASE, Failed!!\n"));
			break;
		} else {
			...
			if (Status) {
				Status  = -EINVAL;
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_SET_PASSPHRASE, Failed (length mismatch)!!\n"));
			} else {
	...
	case RT_OID_802_11_RADIO:
		if (wrq->u.data.length != sizeof(BOOLEAN))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_RADIO (=%d)\n", RadioState));
	...
	case RT_OID_802_11_PHY_MODE:
		if (wrq->u.data.length != sizeof(RT_802_11_PHY_MODE))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_PHY_MODE (=%d)\n", PhyMode));
		}
	...
	case RT_OID_802_11_PREAMBLE:
		if (wrq->u.data.length != sizeof(RT_802_11_PREAMBLE))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_PREAMBLE (=%d)\n", Preamble));
		}
	...
	case OID_802_11_INFRASTRUCTURE_MODE:
		if (wrq->u.data.length != sizeof(NDIS_802_11_NETWORK_INFRASTRUCTURE))
			...
			if (BssType == Ndis802_11IBSS)
				...
				Status  = -EINVAL;
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_INFRASTRUCTURE_MODE (unknown)\n"));
			}
		}
	...
	case OID_802_11_REMOVE_WEP:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_REMOVE_WEP\n"));
...
#ifdef TXBF_SUPPORT
{
...
#endif /* TXBF_SUPPORT */
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_RESET_COUNTERS\n"));
		break;
	...
	case OID_802_11_RTS_THRESHOLD:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_RTS_THRESHOLD (=%ld)\n", RtsThresh));
		break;
	...
	case OID_802_11_FRAGMENTATION_THRESHOLD:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_FRAGMENTATION_THRESHOLD (=%ld)\n", FragThresh));
		break;
	...
	case OID_802_11_POWER_MODE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_POWER_MODE (=%d)\n", PowerMode));
		break;
	...
	case RT_OID_802_11_TX_POWER_LEVEL_1:
		if (wrq->u.data.length  < sizeof(ULONG))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_TX_POWER_LEVEL_1 (BAND0) (=%d)\n",
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_TX_POWER_LEVEL_1 (BAND1) (=%d)\n",
	...
	case OID_802_11_NETWORK_TYPE_IN_USE:
		if (wrq->u.data.length != sizeof(NDIS_802_11_NETWORK_TYPE))
			...
			if (NetType == Ndis802_11DS) {
				...
				hc_set_ChCtrlChListStat(pChCtrl, CH_LIST_STATE_NONE);
				BuildChannelList(pAd, wdev);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[RTMPSetInformation][BandIdx=%d] Ndis802_11DS\n", BandIdx));
				...
				hc_set_ChCtrlChListStat(pChCtrl, CH_LIST_STATE_NONE);
				BuildChannelList(pAd, wdev);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[RTMPSetInformation][BandIdx=%d] Ndis802_11OFDM24\n", BandIdx));
				...
				hc_set_ChCtrlChListStat(pChCtrl, CH_LIST_STATE_NONE);
				BuildChannelList(pAd, wdev);
				MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("[RTMPSetInformation][BandIdx=%d] Ndis802_11OFDM5\n", BandIdx));
				RTMPSetPhyMode(pAd, wdev, WMODE_A);
			}
			else
				Status = -EINVAL;

			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_NETWORK_TYPE_IN_USE (=%d)\n", NetType));
		}
	...
	case RT_OID_802_11_ADD_WPA:
		...
		if ((pKey->Length != wrq->u.data.length) ||
			...
			Status  = -EINVAL;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_ADD_WPA, Failed!!\n"));
		} else {
			if ((!IS_AKM_WPA1PSK(wdev->SecConfig.AKMMap)) &&
				...
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_ADD_WPA (id=0x%x, Len=%d-byte)\n",
	...
	case OID_802_11_ADD_KEY:
		...
		if ((pKey->Length != wrq->u.data.length) ||
			(pKey->KeyLength > LEN_PMK)) {
			Status  = -EINVAL;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_ADD_KEY, Failed!!\n"));
		} else {
			RTMPAddKey(pAd, pKey, wdev);
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_ADD_KEY (id=0x%x, Len=%d-byte)\n",
					 pKey->KeyIndex, pKey->KeyLength));
		}
	...
	case RT_OID_802_11_SET_APSD_PSM:
		if (wrq->u.data.length	!= sizeof(ULONG))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_SET_APSD_PSM (bAPSDForcePowerSave:%d)\n",
	...
	case RT_OID_802_11_SET_WMM:
		if (wrq->u.data.length	!= sizeof(BOOLEAN))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_SET_WMM (=%d)\n", wdev->bWmmCapable));
		}
	...
	case RT_OID_802_11_SET_IMME_BA_CAP:
		if (wrq->u.data.length != sizeof(OID_BACAP_STRUC))
...
#ifdef COEX_SUPPORT
				if (pAd->CommonCfg.BACapability.field.TxBAWinLimit > MAX_RX_REORDERBUF)
					pAd->CommonCfg.BACapability.field.TxBAWinLimit = MAX_RX_REORDERBUF;
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
					 ("Set::(MimoPs = %d)(AmsduSize=%d)(MpduDensity=%d)\n",
					  Orde.MMPSmode,
					  Orde.AmsduSize,
					  Orde.MpduDensity));
		}
	...
	case RT_OID_802_11_ADD_IMME_BA:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" Set :: RT_OID_802_11_ADD_IMME_BA\n"));
		...
		if (wrq->u.data.length != sizeof(OID_ADD_BA_ENTRY))
			...
			if (BA.TID > (NUM_OF_TID - 1)) {
				...
				if (!pEntry) {
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_802_11_ADD_IMME_BA. break on no connection.----:%x:%x\n",
...
#ifdef COEX_SUPPORT
						pAd->CommonCfg.BACapability.field.TxBAWinLimit = 0x10;
#endif /* COEX_SUPPORT */
					}

					ba_ori_session_setup(pAd, pEntry, index, 0, 100, TRUE);
				} else {
	...
	case RT_OID_802_11_TEAR_IMME_BA:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set :: RT_OID_802_11_TEAR_IMME_BA\n"));
		...
		if (wrq->u.data.length != sizeof(OID_ADD_BA_ENTRY))
			...
			if (pBA == NULL) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
						 ("Set :: RT_OID_802_11_TEAR_IMME_BA: can't allocate enough memory\n"));
				...
				if (pBA->IsRecipient == FALSE) {
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" pBA->IsRecipient == FALSE\n"));

					if (pEntry) {
						MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" pBA->pEntry\n"));
						ba_ori_session_tear_down(pAd, pEntry->wcid, pBA->TID, FALSE, TRUE);
					} else
						MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set :: Not found pEntry\n"));
				} else {
					pEntry = MacTableLookup2(pAd, pBA->MACAddr, wdev);

					if (pEntry)
						ba_rec_session_tear_down(pAd, (UCHAR)pEntry->wcid, pBA->TID, TRUE);
					else
						MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set :: Not found pEntry\n"));
				}
	...
	case OID_802_11_ADD_WEP:
		...
		if (pWepKey == NULL) {
			Status = -ENOMEM;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_ADD_WEP, Failed!!\n"));
			break;
		}
		...
		if (Status) {
			Status  = -EINVAL;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::OID_802_11_ADD_WEP, Failed (length mismatch)!!\n"));
		} else {
	...
	case RT_OID_WSC_EAPMSG: {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WSC::RT_OID_WSC_EAPMSG, wrq->u.data.length=%d!\n",
				 wrq->u.data.length));
	...
	case RT_OID_WSC_SET_PROFILE:
		if (wrq->u.data.length != sizeof(WSC_PROFILE))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_SET_PROFILE:: ProfileCnt = %d\n",
	...
	case RT_OID_WSC_SET_CONF_MODE: /* WPS disable, Enrollee or Registrar */
		if (wrq->u.data.length != sizeof(INT))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_SET_CONF_MODE:: WscConfMode = %d\n", WscConfMode));
		}
		...
		if (wrq->u.data.length != sizeof(INT))
			...
			if (Status == 0) {
				if (WscMode == WSC_PIN_MODE) { /* PIN */
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_SET_MODE::unknown WscMode = %d\n", WscMode));
					Status = -EINVAL;
				}
			}

			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_SET_MODE::WscMode = %d\n", WscMode));
		}
	...
	case RT_OID_WSC_DRIVER_AUTO_CONNECT:
		if (wrq->u.data.length != sizeof(UCHAR))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_DRIVER_AUTO_CONNECT::WscDriverAutoConnect is %d\n",
	...
	case RT_OID_WSC_SET_PASSPHRASE:
		if (wrq->u.data.length > 64 || wrq->u.data.length < 8)
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_SET_PASSPHRASE::KeyLen(%d)\n",
	...
	case RT_OID_802_11_SET_PSPXLINK_MODE:
		if (wrq->u.data.length != sizeof(BOOLEAN))
			 ...
			 */
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Set::RT_OID_802_11_SET_PSPXLINK_MODE(=%d)\n",
					 pStaCfg->PSPXlink));
		}
	...
	IN struct wifi_dev *wdev)
{
	...
	case OID_802_11_SSID:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_SSID (Len=%d, ssid=%s)\n", Ssid.SsidLength,
				 Ssid.Ssid));
		break;
	...
	case RT_OID_802_11_QUERY_LINK_STATUS:
		...
		if (pLinkStatus) {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_LINK_STATUS\n"));
		} else {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_LINK_STATUS(mem alloc failed)\n"));
			Status = -EFAULT;
		}
	...
	case OID_802_11_CONFIGURATION:
		...
		if (pConfiguration) {
			...
			MAP_CHANNEL_ID_TO_KHZ(wdev->channel, pConfiguration->DSConfig);
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_CONFIGURATION(mem alloc failed)\n"));
			Status = -EFAULT;
		}
	...
	case RT_OID_802_11_SNR_1:
		if ((pAd->Antenna.field.RxPath > 1) && (pStaCfg->wdev.LastSNR1 > 0)) {
			...
			wrq->u.data.length = sizeof(ulInfo);
			Status = copy_to_user(wrq->u.data.pointer, &ulInfo,	wrq->u.data.length);
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_SNR_1(0x=%lx)\n", ulInfo));
		} else
			Status = -EFAULT;

		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_SNR_1(pStaCfg->LastSNR1=%d)\n",
	...
	case OID_802_11_RSSI_TRIGGER:
		...
		wrq->u.data.length = sizeof(ulInfo);
		Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_RSSI_TRIGGER(=%ld)\n", ulInfo));
		break;
	...
	case RT_OID_802_11_PHY_MODE:
		...
		wrq->u.data.length = sizeof(ulInfo);
		Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_PHY_MODE (=%ld)\n", ulInfo));
		break;
	...
	case RT_OID_802_11_STA_CONFIG:
		...
		if (pStaConfig) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_STA_CONFIG\n"));
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_STA_CONFIG(mem alloc failed)\n"));
			Status = -EFAULT;
		}
	...
	case OID_802_11_RTS_THRESHOLD:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_RTS_THRESHOLD(=%ld)\n", RtsThresh));
		break;
	...
	case OID_802_11_FRAGMENTATION_THRESHOLD:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_FRAGMENTATION_THRESHOLD(=%ld)\n", FragThresh));
		break;
	...
	case OID_802_11_POWER_MODE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_POWER_MODE(=%d)\n", PowerMode));
		break;
	...
	case RT_OID_802_11_RADIO:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_RADIO (=%d)\n", RadioState));
		break;
	...
	case OID_802_11_INFRASTRUCTURE_MODE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_INFRASTRUCTURE_MODE(=%d)\n", BssType));
		break;
	...
	case RT_OID_802_11_PREAMBLE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_PREAMBLE(=%d)\n", PreamType));
		break;
	...
	case OID_802_11_AUTHENTICATION_MODE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_AUTHENTICATION_MODE(=%d)\n", AuthMode));
		break;
	...
	case OID_802_11_TX_POWER_LEVEL:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_TX_POWER_LEVEL %x\n", pAd->CommonCfg.TxPower));
		break;

	case RT_OID_802_11_TX_POWER_LEVEL_1:
		wrq->u.data.length = sizeof(ULONG);
		Status = copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.ucTxPowerPercentage[BAND0], wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_TX_POWER_LEVEL_1 (BAND0) (=%d)\n",
				 pAd->CommonCfg.ucTxPowerPercentage[BAND0]));
#ifdef DBDC_MODE
		Status = copy_to_user(wrq->u.data.pointer, &pAd->CommonCfg.ucTxPowerPercentage[BAND1], wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_TX_POWER_LEVEL_1 (BAND1) (=%d)\n",
				 pAd->CommonCfg.ucTxPowerPercentage[BAND1]));
#endif /* DBDC_MODE */
		break;
	...
	case RT_OID_802_11_QUERY_LAST_RX_RATE:
		...
		wrq->u.data.length = sizeof(ulInfo);
		Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_LAST_RX_RATE (=%ld)\n", ulInfo));
		break;
	...
	case RT_OID_802_11_QUERY_LAST_TX_RATE:
		...
		wrq->u.data.length = sizeof(ulInfo);
		Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_LAST_TX_RATE (=%lx)\n", ulInfo));
		break;
	...
	case RT_OID_802_11_QUERY_MAP_REAL_TX_RATE:
		...
		wrq->u.data.length = sizeof(RateValue);
		Status = copy_to_user(wrq->u.data.pointer, &RateValue, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_LAST_TX_RATE (=%ld)\n", RateValue));
		break;
	...
	case RT_OID_802_11_QUERY_TX_PHYMODE:
		...
		wrq->u.data.length = sizeof(ulInfo);
		Status = copy_to_user(wrq->u.data.pointer, &ulInfo, wrq->u.data.length);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_TX_PHYMODE (=%lx)\n", ulInfo));
		break;
	...
	case RT_OID_802_11_QUERY_NOISE_LEVEL: {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_NOISE_LEVEL (=%d)\n", noise));
	}
	break;
	...
	case RT_OID_802_11_EXTRA_INFO:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_EXTRA_INFO (=%ld)\n", pAd->ExtraInfo));
		break;
	...
	case RT_OID_802_11_QUERY_APSD_PSM:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_QUERY_APSD_PSM (=%d)\n",
				 pAd->CommonCfg.bAPSDForcePowerSave));
		break;
	...
	case RT_OID_WSC_QUERY_STATUS:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_WSC_QUERY_STATUS (=%d)\n",
	...
	case RT_OID_WSC_QUERY_DEFAULT_PROFILE:
		...
		if (pProfile == NULL) {
			Status = -EFAULT;
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("RT_OID_WSC_QUERY_DEFAULT_PROFILE fail!\n"));
			break;
		}
		...
		if (copy_to_user(wrq->u.data.pointer, pProfile, wrq->u.data.length))
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WSC Profile:\n"));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("SSID = %s\n", pProfile->Profile[0].SSID.Ssid));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("AuthType = %s\n",
					 WscGetAuthTypeStr(pProfile->Profile[0].AuthType)));
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("EncrpType = %s\n",
					 WscGetEncryTypeStr(pProfile->Profile[0].EncrType)));

			if (pProfile->Profile[0].EncrType == WSC_ENCRTYPE_WEP) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("WEP Key = %s\n", pProfile->Profile[0].Key));
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("DefaultKey ID = %d\n", pProfile->Profile[0].KeyIndex));
			} else if ((pProfile->Profile[0].EncrType == WSC_ENCRTYPE_TKIP) ||
					   (pProfile->Profile[0].EncrType == WSC_ENCRTYPE_AES)) {
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("PassPhrase Key = %s\n", pProfile->Profile[0].Key));
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_WSC_QUERY_DEFAULT_PROFILE\n"));
		break;
	...
	case RT_OID_DRIVER_DEVICE_NAME:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_DRIVER_DEVICE_NAME\n"));
		wrq->u.data.length = 16;
	...
	case RT_OID_802_11_QUERY_HT_PHYMODE:
		...
		if (pHTPhyMode) {
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, (" %s(): (.word = %x )\n",
					 __func__, pEntry->HTPhyMode.word));
	...
	case RT_OID_802_11_COUNTRY_REGION:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::RT_OID_802_11_COUNTRY_REGION\n"));
		wrq->u.data.length = sizeof(ulInfo);
	...
	case OID_802_11_BUILD_CHANNEL_EX: {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_BUILD_CHANNEL_EX\n"));
		wrq->u.data.length = sizeof(UCHAR);
#ifdef EXT_BUILD_CHANNEL_LIST
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Support EXT_BUILD_CHANNEL_LIST.\n"));
		value = 1;
#else
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Doesn't support EXT_BUILD_CHANNEL_LIST.\n"));
		value = 0;
#endif /* EXT_BUILD_CHANNEL_LIST */
	...
	case OID_802_11_GET_CH_LIST: {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_GET_CH_LIST\n"));
	...
	case OID_802_11_GET_COUNTRY_CODE:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_GET_COUNTRY_CODE\n"));
		wrq->u.data.length = 2;
	...
	case OID_802_11_GET_CHANNEL_GEOGRAPHY:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_GET_CHANNEL_GEOGRAPHY\n"));
		wrq->u.data.length = 1;
	...
	case OID_802_11_SET_PSPXLINK_MODE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11_SET_PSPXLINK_MODE(=%d)\n", pStaCfg->PSPXlink));
		break;
#endif /* XLINK_SUPPORT */
#ifdef DOT11R_FT_SUPPORT
	...
	case OID_802_11R_SUPPORT:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11R_SUPPORT(=%d)\n",
				 pStaCfg->Dot11RCommInfo.bFtSupport));
		break;
	...
	case  OID_802_11R_MDID:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::OID_802_11R_MDID(=%x%x)\n",
	...
	default:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Query::unknown IOCTL's subcmd = 0x%08x\n", cmd));
		Status = -EOPNOTSUPP;
		break;
	}
...
range:
	...
	if (bIsRange) {
		...
		if (e2pAddrEnd > e2pAddrStart) {
			...
			if (pMacContent == NULL)
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("-->2) %s: Allocate memory fail!\n", __func__));
			else {
				/* get MAC content */
				USHORT eepValue;
				...
				while (e2pAddrStart <= e2pAddrEnd) {
					...
					*pMacIndex = eepValue;
					...
					MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("eepAddr=%08x, eepValue=%04x\n", e2pAddrStart, eepValue));
					pMacIndex++;
					e2pAddrStart += 2;
				}
	...
	IN	ULONG					Data)
{
	...
	if (copy_from_user(this_char, wrq->u.data.pointer, wrq->u.data.length)) {
		...
		case SHOW_WTBL_INFO_TLV:
			show_wtbl_proc_ByTlv(pAd, this_char);
			wrq->u.data.length = 0;
			break;
#endif
		...
		case SHOW_E2P_INFO:
			...
			break;
		default:
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - unknow subcmd = %d\n", __func__, subcmd));
			break;
		}
	...
	IN	ULONG					Data)
{
	...
	if (ChannelSanity(pAd, chan) == TRUE) {
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("==>rt_ioctl_siwfreq::SIOCSIWFREQ(Channel=%d)\n", wdev->channel));
	} else
		return NDIS_STATUS_FAILURE;
	...
	IN	ULONG					Data)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>rt_ioctl_giwfreq  %d\n", ch));
	...
	IN	ULONG					Data)
{
	...
	else if (INFRA_ON(&pAd->StaCfg[pObj->ioctl_if]))
	...
	IN	ULONG					Data)
{
	...
	if (STA_STATUS_TEST_FLAG(pStaCfg, fSTA_STATUS_MEDIA_STATE_CONNECTED)) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MediaState is connected\n"));
		...
		pSsid->SsidLen = 0;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MediaState is not connected, ess\n"));
	}
...
INT RtmpIoctl_rt_ioctl_siwencode(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>rt_ioctl_siwencode::erq->flags=%x\n", pIoctlSec->flags));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>rt_ioctl_siwencode::AuthMode=0x%x\n",
			 wdev->SecConfig.AKMMap));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>rt_ioctl_siwencode::DefaultKeyId=%x, KeyLen = %d\n",
			 wdev->SecConfig.PairwiseKeyId, wdev->SecConfig.WepKey[wdev->SecConfig.PairwiseKeyId].KeyLen));
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==>rt_ioctl_siwencode::WepStatus=0x%x\n",
...
INT RtmpIoctl_rt_ioctl_giwencode(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data)
{
	...
	MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("===>rt_ioctl_giwencode %d\n", kid));
	...
	IN	UINT32					Subcmd)
{
	...
	case RT_CMD_STA_IOCTL_IW_MLME_DEAUTH:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("====> %s - IW_MLME_DEAUTH\n", __func__));
		cntl_disconnect_request(wdev,
								CNTL_DEAUTH,
								pStaCfg->Bssid,
								(USHORT)reason_code,
	...
	case RT_CMD_STA_IOCTL_IW_MLME_DISASSOC:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("====> %s - IW_MLME_DISASSOC\n", __func__));
	...
	IN	ULONG					Data)
{
	...
	case RT_CMD_STA_IOCTL_WPA_VERSION:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_WPA_VERSION - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;
	...
	case RT_CMD_STA_IOCTL_WPA_PAIRWISE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_CIPHER_PAIRWISE - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;
	...
	case RT_CMD_STA_IOCTL_WPA_GROUP:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_CIPHER_GROUP - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;
	...
	case RT_CMD_STA_IOCTL_WPA_KEY_MGMT:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_KEY_MGMT - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;
	...
	case RT_CMD_STA_IOCTL_WPA_AUTH_PRIVACY_INVOKED:
		 ...
		 *}
		 */
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_PRIVACY_INVOKED - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;
	...
	case RT_CMD_STA_IOCTL_WPA_AUTH_DROP_UNENCRYPTED:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_DROP_UNENCRYPTED - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;

	case RT_CMD_STA_IOCTL_WPA_AUTH_80211_AUTH_ALG:
		SET_AKM_AUTOSWITCH(wdev->SecConfig.AKMMap);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_AUTH_80211_AUTH_ALG - param->value = %d!\n", __func__,
				 pIoctlWpa->value));
		break;
...
INT RtmpIoctl_rt_ioctl_siwencodeext(RTMP_ADAPTER *pAd, VOID *pData, ULONG Data, struct wifi_dev *wdev)
{
	...
	/* pEntry = &pAd->MacTab.Content[BSSID_WCID_TO_REMOVE]; */
	{
		...
		case RT_CMD_STA_IOCTL_SECURITY_ALG_NONE:
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_ENCODE_ALG_NONE\n", __func__));
			break;

		case RT_CMD_STA_IOCTL_SECURITY_ALG_WEP:
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_ENCODE_ALG_WEP - ext->key_len = %d, keyIdx = %d\n",
		...
		case RT_CMD_STA_IOCTL_SECURITY_ALG_TKIP:
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::IW_ENCODE_ALG_TKIP - keyIdx = %d, ext->key_len = %d\n",
	...
	IN	ULONG					Data)
{
	...
	case RT_CMD_STA_IOCTL_PMA_SA_FLUSH:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwpmksa - IW_PMKSA_FLUSH\n"));
		break;
	...
	case RT_CMD_STA_IOCTL_PMA_SA_REMOVE:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwpmksa - IW_PMKSA_REMOVE\n"));
		break;
	...
	case RT_CMD_STA_IOCTL_PMA_SA_ADD:
		...
		if (CachedIdx < PMKID_NO) {
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("Update PMKID, idx = %d\n", CachedIdx));
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwpmksa - IW_PMKSA_ADD\n"));
		break;

	default:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("rt_ioctl_siwpmksa - Unknow Command!!\n"));
		break;
	}
	...
	IN	ULONG					Data)
{
	...
	memcpy(pData, pAd->StaCfg[pObj->ioctl_if].wdev.if_addr, ETH_ALEN);
	return NDIS_STATUS_SUCCESS;
}
	...
	IN	ULONG					Data)
{
	...
	case WSC_CREDENTIAL_COUNT:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_COUNT, value = %d\n", __func__, value));
	...
	case WSC_SET_DRIVER_CONNECT_BY_CREDENTIAL_IDX:
		value = *(pIoctlWscU32->pUWrq + 1);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_DRIVER_CONNECT_BY_CREDENTIAL_IDX, value = %d\n",
				 __func__, value));
	...
	case WSC_SET_DRIVER_AUTO_CONNECT:
		value = *(pIoctlWscU32->pUWrq + 1);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_DRIVER_AUTO_CONNECT, value = %d\n", __func__,
				 value));
	...
	case WSC_SET_CONF_MODE:
		value = *(pIoctlWscU32->pUWrq + 1);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_CONF_MODE, value = %d\n", __func__, value));
	...
	case WSC_SET_MODE:
		value = *(pIoctlWscU32->pUWrq + 1);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_MODE, value = %d\n", __func__, value));
	...
	case WSC_START:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_START\n", __func__));
		Set_WscGetConf_Proc(pAd, "1");
		break;

	case WSC_STOP:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_STOP\n", __func__));
	...
	case WSC_GEN_PIN_CODE:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_GEN_PIN_CODE\n", __func__));
		Set_WscGenPinCode_Proc(pAd, "1");
		break;

	case WSC_AP_BAND:
		value = *(pIoctlWscU32->pUWrq + 1);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_PBC_BAND, value = %d\n", __func__, value));
	...
	default:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - unknow subcmd = %d, value = %d\n", __func__, subcmd,
				 value));
		break;
	}
	...
	IN	ULONG					Data)
{
	...
	if ((subcmd != WSC_SET_SSID) &&
		...
		(tmpProfileIndex > 7)) {
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - subcmd = %d, tmpProfileIndex = %d\n", __func__, subcmd,
				 tmpProfileIndex));
	...
	case WSC_CREDENTIAL_SSID:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_SSID(%s)\n", __func__, extra + 2));
	...
	case WSC_CREDENTIAL_AUTH_MODE:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_AUTH_MODE(%s)\n", __func__, extra + 2));
	...
	case WSC_CREDENTIAL_ENCR_TYPE:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_ENCR_TYPE(%s)\n", __func__, extra + 2));
	...
	case WSC_CREDENTIAL_KEY_INDEX:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_KEY_INDEX(%s)\n", __func__, extra + 2));
	...
	case WSC_CREDENTIAL_KEY:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_KEY(%s)\n", __func__, extra + 2));
	...
	case WSC_CREDENTIAL_MAC:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_CREDENTIAL_MAC(%s)\n", __func__, extra + 2));
		{
			INT sscanf_rv = 0;
			UINT tmp_val[6] = {0};
	...
	case WSC_SET_SSID:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_SSID(%s)\n", __func__, extra));
	...
	case WSC_SET_PIN:
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_PIN, value = (%s)\n", __func__, extra));
	...
	case WSC_SET_BSSID:
		...
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s - WSC_SET_BSSID\n", __func__));
		break;
	...
	IN	ULONG					Data)
{
	...
	{
#ifdef ENHANCED_STAT_DISPLAY
		sprintf(extra + strlen(extra), "RSSI                            = %ld %ld %ld %ld\n",
	...
	IN  USHORT                  priv_flags)
{
	...
	case CMD_RTPRIV_IOCTL_STA_SCAN_END:
		...
		pStaCfg->bSkipAutoScanConn = FALSE;
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("===>rt_ioctl_giwscan. %d(%d) BSS returned, data->length = %ld\n",
