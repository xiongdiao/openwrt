...
BOOLEAN rtmp_ee_flash_eeprom_default_file_path(PRTMP_ADAPTER pAd, PUCHAR *path)
{
	static UCHAR default_bin_path[64];

	NdisZeroMemory(default_bin_path, sizeof(default_bin_path));

		sprintf(default_bin_path, EEPROM_DEFAULT_FILE_PATH);

	*path = &default_bin_path[0];
...
static NDIS_STATUS rtmp_ee_flash_reset(RTMP_ADAPTER *pAd, UCHAR *start)
{
	...
	rtmp_ee_flash_eeprom_default_file_path(pAd, &src);
...
#ifdef MT7663
	if (IS_MT7663(pAd)) {
		if ((eeFlashId == 0x7663) || (eeFlashId == 0x7613))
			return TRUE;
	} else
#endif
