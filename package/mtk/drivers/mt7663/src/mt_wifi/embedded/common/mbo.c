...
/* tmply hard code */
	case MBO_ATTR_STA_NOT_PREFER_CH_REP:
		...
		MboAttr.AttrLen	= 0x05;
		MboAttr.AttrBody[0] = 0x0c;
		MboAttr.AttrBody[1] = 0x01;
		MboAttr.AttrBody[2] = 0x06;
		MboAttr.AttrBody[3] = 0x01;
		MboAttr.AttrBody[4] = 0x01;
		break;
	case MBO_ATTR_STA_NOT_PREFER_CH_REP_2ND:
		MboAttr.AttrID		= MBO_ATTR_STA_NOT_PREFER_CH_REP;
		MboAttr.AttrLen	= 0x04;
		MboAttr.AttrBody[0] = 0x73;
		MboAttr.AttrBody[1] = 0x2C;
		MboAttr.AttrBody[2] = 0x10;
		MboAttr.AttrBody[3] = 0x01;
		break;
		...
		for (loop = 0; loop < ScanTab->BssNr; loop++) {/* minus our own NR entry */
			...
			&& (NeighborRepList.CurrNum != 0)
				...
				printk("\033[1;32m %s, %u NeighborRepList.Newlist %d NeighborRepList.TotalNum %d NeighborRepList.CurrNum %d\033[0m\n"
					, __func__, __LINE__, NeighborRepList.Newlist, NeighborRepList.TotalNum, NeighborRepList.CurrNum);
				NdisZeroMemory(&NeighborRepList, sizeof(DAEMON_EVENT_NR_LIST));
				NeighborRepList.TotalNum = TotalReportNum;
			}
...
INT32 ShowMboStatProc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	for (loop = 0; loop < MAX_MULTI_STA; loop++) {
		pMboCtrl = &pAd->StaCfg[loop].wdev.MboCtrl;
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("========= StaCfg[%d] ===========\n", loop));
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("bMboEnable \t\t %d\n",
			pMboCtrl->bMboEnable));
	}
#endif /* CONFIG_STA_SUPPORT */
