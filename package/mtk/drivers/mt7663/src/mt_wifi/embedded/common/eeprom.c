...
INT NICReadEEPROMParameters(RTMP_ADAPTER *pAd, RTMP_STRING *mac_addr)
{
...
#if defined(PRE_CAL_TRX_SET2_SUPPORT) || defined(PRE_CAL_MT7622_SUPPORT) || defined(PRE_CAL_MT7663_SUPPORT) || defined(PRE_CAL_MT7626_SUPPORT)
	UINT16 DoPreCal = 0;
#endif /* PRE_CAL_TRX_SET2_SUPPORT */
#if defined(CAL_BIN_FILE_SUPPORT) && defined(MT7615)
	UINT16 DoPATrim = 0;
			...
			if (DoPreCal & (1 << 0)) {
				...
				while (setPreCalTotal > 0) {
					if (setPreCalTotal >= PRE_CAL_SET_MAX_LENGTH)
						MtCmdSetGroupPreCal_7663(pAd, setPreCalIdx, PRE_CAL_SET_MAX_LENGTH);
					else
						MtCmdSetGroupPreCal_7663(pAd, setPreCalIdx, setPreCalTotal);
...
#endif
#ifdef MT_MAC
#if defined(MT7603) || defined(MT7628) || defined(MT7636) || defined(MT7615) || defined(MT7637) || defined(MT7622) || defined(MT7663) || defined(MT7626)
...
#endif
	{
		...
		if (IS_MT7603(pAd) || IS_MT7628(pAd) || IS_MT7636(pAd) || IS_MT7615(pAd) || IS_MT7637(pAd) || IS_MT7622(pAd) || IS_MT7663(pAd) || IS_MT7626(pAd))
...
#endif /*#if defined(MT7603)||defined(MT7628)||defined(MT7636))||defined(MT7615)||defined(MT7622) || defined(MT7663)*/
#endif /* MT_MAC */
	...
	IN PRTMP_ADAPTER	pAd)
{
	...
	if (ops && ops->get_bin_image_file != NULL) {
		src = ops->get_bin_image_file(pAd);
		MTWF_LOG(DBG_CAT_HW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			("%s::bin FileName=%s\n", __func__, src));
	} else {
		src = BIN_FILE_PATH;
...
INT Set_bufferMode_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (EepromType == EEPROM_FLASH)
		EepromType = EEPROM_FLASH_CMD;
