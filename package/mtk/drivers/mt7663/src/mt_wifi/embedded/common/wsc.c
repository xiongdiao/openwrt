	...
	IN  MLME_QUEUE_ELEM * Elem)
{
...
#ifdef CON_WPS
	{
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("CON_WPS: Stop the ApCli WPS, state [%d]\n", pApCliWpsCtrl->WscState));

		if ((pApCliWpsCtrl->conWscStatus != CON_WPS_STATUS_DISABLED) &&
	...
	if (MsgType == WSC_MSG_EAP_REG_RSP_ID) {
		...
		if (bUPnPMsg) {
			...
			if ((pWscControl->WscMode == WSC_PBC_MODE) &&
				...
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("WscEAPAction: PBC Session Overlap!\n"));
		...
		if (pWscControl->WscState >= WSC_STATE_WAIT_EAPFAIL) {
			...
			if ((CurOpMode == STA_MODE) &&
				...
				if (pEntry && IS_ENTRY_PEER_AP(pEntry) && pAdapter->StaCfg[if_idx].ApcliInfStat.Enable == TRUE
					...
					pAdapter->StaCfg[if_idx].ApcliInfStat.Enable = FALSE;
					ApCliIfDown(pAdapter);
#ifdef CONFIG_MAP_SUPPORT
					if (!IS_MAP_TURNKEY_ENABLE(pAdapter))
#endif /* CONFIG_MAP_SUPPORT */
					pAdapter->StaCfg[if_idx].ApcliInfStat.Enable = TRUE;
				}
			}
		...
		*/
		...
		if (((pWscControl->WscConfMode & WSC_PROXY) != 0) && (!bUPnPMsg) && (CurOpMode == AP_MODE)) {
			...
			if ((rv == 0) && (CurOpMode == STA_MODE)) {
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("(Line:%d)Ignore EAP M1.\n", __LINE__));
				goto out;
			}
		}
...
Done:
	...
	if (rv == 1) {
		...
		RTMPSetLED(pAdapter, WPSLEDStatus, HcGetBandByWdev((struct wifi_dev *)pWscControl->wdev));
#endif /* WSC_LED_SUPPORT */
	}
}
	...
	IN  PMLME_QUEUE_ELEM pElem)
{
	...
	} else if (pElem->MsgLen && NdisEqualMemory(id_data, pElem->Msg, pElem->MsgLen)) {
		/* Eap-Req/Identity(hello) */
		return  WSC_MSG_EAP_REQ_ID;
	} else if (pElem->MsgLen && NdisEqualMemory(fail_data, pElem->Msg, pElem->MsgLen)) {
		/* Eap-Fail */
		return  WSC_MSG_EAP_FAIL;
	} else if (pElem->MsgLen && NdisEqualMemory(wsc_start, pElem->Msg, pElem->MsgLen)) {
		/* Eap-Req(Wsc_Start) */
		return WSC_MSG_EAP_REQ_START;
	}

#ifdef WSC_V2_SUPPORT
	else if (pElem->MsgLen && NdisEqualMemory(wsc_frag_ack, pElem->Msg, pElem->MsgLen)) {
		/* WSC FRAG ACK */
	...
	IN  RTMP_STRING *arg)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscSetAuthMode::(AuthMode=0x%x)\n", wdev->SecConfig.AKMMap));
	}
	...
	IN  RTMP_STRING *arg)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		pSecConfig = &wdev->SecConfig;
		SetWdevEncrypMode(pSecConfig, arg);
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscSetEncrypType::(EncrypType=0x%x)\n", wdev->SecConfig.PairwiseCipher));
	}
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	if (!cntl_idle(wdev)) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("!!! WscPBCExec --> MLME busy, reset MLME state machine !!!\n"));
	}

#endif /* CONFIG_STA_SUPPORT */
#ifndef APCLI_SUPPORT
	/* For ApCli this would be done in CMDTHREAD_APCLI_PBC_AP_FOUND handle */
	pWscControl->WscState = WSC_STATE_START;
	...
	IN  PWSC_CTRL pWscControl)
{
	...
	if (!cntl_idle(&pAd->StaCfg[0].wdev)) {
		...
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("!!! WscPINExec --> MLME busy, reset MLME state machine !!!\n"));
	}
	...
	if (CurOpMode == STA_MODE) {
		pWscControl->WscState = WSC_STATE_START;
		pWscControl->WscStatus = STATUS_WSC_START_ASSOC;
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_OFF, ("wsc_state=start !!!\n"));
	}
	...
	if (CurOpMode == STA_MODE) {
		if (pStaCfg->BssType == BSS_INFRA) {
			MlmeEnqueueWithWdev(pAd,
								...
								(VOID *)&pWscControl->WscBssid[0], 0, &pStaCfg->wdev, NULL);
		} else {
			MlmeEnqueueWithWdev(pAd,
								MLME_CNTL_STATE_MACHINE,
								OID_802_11_SSID,
								sizeof(NDIS_802_11_SSID),
								(VOID *)&pWscControl->WscSsid, 0, &pStaCfg->wdev, NULL);
		}
	}
	...
	PUCHAR pVar)
{
	...
	while ((Len > 0) && (bFound == FALSE)) {
		...
		if (Reqwdev && IS_MAP_TURNKEY_ENABLE(pAd)
			&& (HcGetBandByChannel(pAd, pInBss->Channel) != HcGetBandByWdev(Reqwdev))) {
			if (bFound == TRUE)
				MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR,
...
#ifdef CONFIG_STA_SUPPORT
	if (wdevBand != RcGetBandIdxByChannel(pAd->hdev_ctrl, pInBss->Channel))
			continue;
	...
	IN  PVOID wdev_obj)
{
	...
	if (INFRA_ON(sta_cfg) && STA_STATUS_TEST_FLAG(sta_cfg, fSTA_STATUS_MEDIA_STATE_CONNECTED)) {
		MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscLinkDown(): Disassociate with current WPS AP...\n"));
		cntl_disconnect_request(wdev, CNTL_DISASSOC, sta_cfg->Bssid, REASON_DISASSOC_STA_LEAVING, NULL);
	...
	IN  PRTMP_ADAPTER pAd)
{
	...
	MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("WscPBCSessionOverlapCheck : WscPBCStaProbeCount = %d\n",
			 pWscStaPbcProbeInfo->WscPBCStaProbeCount));
}
	...
	IN  RTMP_STRING *pTempStr)
{
	...
	if (CurOpMode == STA_MODE)
		snprintf(pTempStr, 512, "%s%d", pTempStr, pAd->StaCfg[0].wdev.SecConfig.PairwiseKeyId + 1);
...
VOID WscWriteConfToDatFile(RTMP_ADAPTER *pAd, UCHAR CurOpMode)
{
	...
	if (CurOpMode == STA_MODE) {
		...
		snprintf(WepKeyName, sizeof(WepKeyName), "Key%dStr=", pAd->StaCfg[0].wdev.SecConfig.PairwiseKeyId + 1);
		snprintf(WepKeyFormatName, sizeof(WepKeyFormatName), "Key%dType=", pAd->StaCfg[0].wdev.SecConfig.PairwiseKeyId + 1);
	}
	...
	IN PVOID SystemSpecific3)
{
	...
	if (pReg->PeerInfo.map_DevPeerRole & BIT(MAP_ROLE_BACKHAUL_STA)) {
		...
		if (bh_wdev) {
			...
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_ERROR, ("bh_wdev is NULL\n"));
			return;
		}
	}
#endif
...
INT WscStaShowPeerList(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (pWscEnList->size != 0) {
		...
		while (pPeerEntry != NULL) {
			MTWF_LOG(DBG_CAT_SEC, CATSEC_WPS, DBG_LVL_TRACE, ("MAC:%02x:%02x:%02x:%02x:%02x:%02x\tReveive Time:%lu\n",
					 pPeerEntry->mac_addr[0],
					 pPeerEntry->mac_addr[1],
					 pPeerEntry->mac_addr[2],
					 pPeerEntry->mac_addr[3],
