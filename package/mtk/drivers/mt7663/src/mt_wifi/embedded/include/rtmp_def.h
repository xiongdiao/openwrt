...
enum WIFI_MODE {
	...
	WMODE_AC = 1 << 5,
	WMODE_AX = 1 << 6,
	WMODE_COMP = 7,	/* total types of supported wireless mode, add this value once yow add new type */
};
...
#define WMODE_CAP_AX(_x) (((_x) & WMODE_AX) != 0)
...
#define fSTA_STATUS_APCLI_MAIN_LINK_DOWN_IN_PROGRESS    0x00000100
...
#define WDEV_NUM_MAX		(HW_BEACON_MAX_NUM + MAX_WDS_ENTRY + \
							MAX_MULTI_STA + MAX_P2P_NUM + MAX_MESH_NUM)
...
#define MAX_MULTI_STA_DEFAULT	2
#define MAX_MULTI_STA   MAX_MULTI_STA_DEFAULT
#ifdef APCLI_CONNECTION_TRIAL
#undef MAX_MULTI_STA
#define MAX_MULTI_STA   (MAX_MULTI_STA_DEFAULT+1)
#endif /*APCLI CONNECTION TRIAL */
...
#if defined(MT7663)
#define MAX_LEN_OF_MAC_TABLE 136 /* static size about MacTab in compiler time */
#else
...
#if defined(DOT11W_PMF_SUPPORT) || defined(CONFIG_DOT11V_WNM)
...
#define RTMP_L2_FRAME_TX_ACTION(__pAd, __ApIdx, __FrameBuf, __FrameLen)		\
	RTMPL2FrameTxAction(__pAd, get_netdev_from_bssid(__pAd, __ApIdx),		\
						announce_802_3_packet_cmm, __ApIdx, __FrameBuf, __FrameLen, __pAd->OpMode)
