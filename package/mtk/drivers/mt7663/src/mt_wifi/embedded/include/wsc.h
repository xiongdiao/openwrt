...
#define WSC_HDR_BTN_MR_PRESS_FLG_GET(__pAd, __FlgIsPressed)				\
	{																	\
		...
		if (IS_HIF_TYPE(__pAd, HIF_MT)) {\
			...
			RTMP_IO_READ32(__pAd->hdev_ctrl, GPIO_CTRL_CFG, (&__gpio_value));			\
...
#define DISASSOC_WAIT_EAP_SUCCESS RTMPMsecsToJiffies(200)
...
typedef	struct	_WSC_CTRL {
...
#ifdef CONFIG_STA_SUPPORT
	UCHAR					WscPINBssCount;			/* Count of PIN activated APs. */
	...
	UCHAR					ScanCountToincludeWPSPin1;	/* if count is more than 3, include wsc v 1.0 registrars to the available WPS AP list for connection*/
#endif /* CONFIG_STA_SUPPORT */
	...
	INT con_wps_scan_trigger_count;
	...
	char                    IfName[IFNAMSIZ];
#ifdef CONFIG_MAP_SUPPORT
	BOOLEAN				entryBackhaulSta;
#endif
	RTMP_OS_COMPLETION		WscEAPHandshakeCompleted;
}	WSC_CTRL, *PWSC_CTRL;
