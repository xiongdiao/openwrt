...
#define COPY_SETTINGS_FROM_MLME_AUX_TO_ACTIVE_CFG(_pAd, _pStaCfg, _pEntry)                                 \
	{                                                                                       \
		...
		(_pStaCfg)->StaActive.SupRateLen = (_pStaCfg)->MlmeAux.SupRateLen;                          \
		NdisMoveMemory((_pStaCfg)->StaActive.SupRate, (_pStaCfg)->MlmeAux.SupRate, (_pStaCfg)->MlmeAux.SupRateLen);\
		(_pStaCfg)->StaActive.ExtRateLen = (_pStaCfg)->MlmeAux.ExtRateLen;                          \
		NdisMoveMemory((_pStaCfg)->StaActive.ExtRate, (_pStaCfg)->MlmeAux.ExtRate, (_pStaCfg)->MlmeAux.ExtRateLen);\
		...
		(_pEntry)->RateLen = (_pStaCfg)->StaActive.SupRateLen + (_pStaCfg)->StaActive.ExtRateLen;\
	}
...
static VOID join_iterate_by_cfg(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	/* retry Probe Req. */
	MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_TRACE, ("(%s) Retry Probe Req.\n", __func__));
...
#endif /* WSC_AP_SUPPORT */
	{
		...
		if (IS_AKM_OWE(pStaCfg->wdev.SecConfig.AKMMap) && (pStaCfg->owe_trans_ssid_len != 0)) {
			JoinReq.SsidLen = pStaCfg->owe_trans_ssid_len;
			NdisMoveMemory(&(JoinReq.Ssid), pStaCfg->owe_trans_ssid, JoinReq.SsidLen);
			COPY_MAC_ADDR(JoinReq.Bssid, pStaCfg->owe_trans_bssid);
		} else
#endif
	...
	USHORT reason)
{
	...
	if (Elem) {
		...
		Elem->priv_data.rept_cli_idx = NON_REPT_ENTRY;
#ifdef MAC_REPEATER_SUPPORT
		Elem->priv_data.rept_cli_entry = NULL;
	...
	UCHAR *data)
{
	...
	if (data_len != sizeof(NDIS_802_11_SSID)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s():: data_len %d != sizeof(NDIS_802_11_SSID)\n",
				  __func__, data_len));
		return;
	}
	...
	UCHAR *data)
{
		...
		if (((pStaCfg->BssType == BSS_INFRA) && (!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS))) ||
			...
			if ((pStaCfg->ConnectinfoChannel  != 0) && (pStaCfg->Connectinfoflag == TRUE)) {
				...
				MlmeEnqueueWithWdev(pAd, SYNC_FSM,  SYNC_FSM_JOIN_REQ,
									sizeof(MLME_JOIN_REQ_STRUCT), &JoinReq, 0, wdev, NULL);
				cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_WAIT_SYNC, __func__);
			} else	{
				ScanParmFill(pAd, &ScanReq, (RTMP_STRING *) pStaCfg->MlmeAux.Ssid,
							 pStaCfg->MlmeAux.SsidLen, BSS_ANY, SCAN_ACTIVE);
				MlmeEnqueueWithWdev(pAd, SYNC_FSM, SYNC_FSM_SCAN_REQ,
									sizeof(MLME_SCAN_REQ_STRUCT), &ScanReq, 0, wdev, NULL);
				cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_WAIT_SYNC, __func__);
			}
		} else {
			...
			StartParmFill(pAd, &StartReq, (PCHAR)pStaCfg->MlmeAux.Ssid,
						  pStaCfg->MlmeAux.SsidLen);
			MlmeEnqueueWithWdev(pAd, SYNC_FSM, SYNC_FSM_SCAN_REQ,
								sizeof(MLME_START_REQ_STRUCT), &StartReq, 0, wdev, NULL);
			cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_WAIT_SYNC, __func__);
		}
	...
	os_move_mem(&pStaCfg->MlmeAux.SsidBssTab.BssEntry[0], pInBss,
				sizeof(BSS_ENTRY));
	{
		if (INFRA_ON(pStaCfg)) {
...
#endif
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - Roaming in MlmeAux.RoamTab...\n"));
#ifdef DOT11R_FT_SUPPORT
	...
	if (pStaCfg->Dot11RCommInfo.bFtSupport &&
		...
		if (pEntry->MdIeInfo.FtCapPlc.field.FtOverDs) {
			...
			MlmeEnqueueWithWdev(pAd, FT_OTD_ACT_STATE_MACHINE,
								FT_OTD_MT2_MLME_REQ,
								sizeof(MLME_FT_REQ_STRUCT), &FtReq, 0, wdev, NULL);
		} else
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("CNTL - Shall not haapen!!\n"));
	} else
#endif /* DOT11R_FT_SUPPORT */
	{
	...
	UINT32 data_len)
{
	...
	default:
		MTWF_LOG(DBG_CAT_MLME, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				 ("%s: Unknow conn_type(=%d)\n",
				  __func__, cntl_conn->conn_type));
		break;
	}
}
	...
	VOID *elem_obj)
{
	...
	UCHAR cli_idx = NON_REPT_ENTRY;

	Elem = (MLME_QUEUE_ELEM *)elem_obj;
	wdev = Elem->wdev;
	pAd = (PRTMP_ADAPTER)wdev->sys_handle;
	CNTL_GET_STA_CFG(pAd, wdev, pStaCfg);
	cli_idx = Elem->priv_data.rept_cli_idx;
	...
	if (cntl_disconn->cntl_disconn_type == CNTL_DEAUTH) {
		MlmeEnqueueWithWdev(pAd, AUTH_FSM, AUTH_FSM_MLME_DEAUTH_REQ,
							sizeof(MLME_DISCONNECT_STRUCT), &mlme_disconn, 0, wdev, &Elem->priv_data);
		cntl_fsm_state_transition(wdev, cli_idx, CNTL_WAIT_DEAUTH, __func__);
		RTMP_MLME_HANDLER(pAd);
	} else if (cntl_disconn->cntl_disconn_type == CNTL_DISASSOC) {
		MlmeEnqueueWithWdev(pAd, ASSOC_FSM, ASSOC_FSM_MLME_DISASSOC_REQ,
							sizeof(MLME_DISCONNECT_STRUCT), &mlme_disconn, 0, wdev, &Elem->priv_data);
		cntl_fsm_state_transition(wdev, cli_idx, CNTL_WAIT_DISASSOC, __func__);
		RTMP_MLME_HANDLER(pAd);
	}
...
#endif /* CONFIG_STA_ADHOC_SUPPORT	*/
		{
			...
			if ((IS_AKM_WPA3PSK(pStaCfg->wdev.SecConfig.AKMMap)) && (pAPEntry && (IS_AKM_WPA3PSK(pAPEntry->SecConfig.AKMMap)))) {
				...
				if (sae_get_pmk_cache(&pAd->SaeCfg, if_addr, pStaCfg->MlmeAux.Bssid, pmkid, pmk)) {
					sta_add_pmkid_cache(pAd, pStaCfg->MlmeAux.Bssid, pmkid, pmk, LEN_PMK,  pStaCfg->wdev.func_idx, CliIdx);
					has_pmkid = TRUE;
				}
...

			{
				...
				if (CliIdx != NON_REPT_ENTRY) {
					...
					MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_INFO,
							 ("(%s) Repeater Cli Trigger Auth Req CliIdx = %d !!!\n",
							  __func__, CliIdx));
				} else
#endif /* MAC_REPEATER_SUPPORT */
				{
				...
				if (IS_SECURITY_SHARED_WEP_Entry(&pStaCfg->wdev))
#else

				/* either Ndis802_11AuthModeShared or Ndis802_11AuthModeAutoSwitch, try shared key first */
				if (IS_SECURITY_SHARED_WEP_Entry(&pStaCfg->wdev)
					|| IS_SECURITY_AUTOSWITCH_Entry(&pStaCfg->wdev))
			...
			cntl_fsm_state_transition(wdev, CliIdx, CNTL_WAIT_AUTH, __func__);
		}
	} else if (Reason == MLME_INVALID_FORMAT) {
	  /* Do Not thing */
	  cntl_fsm_state_transition(wdev, CliIdx, CNTL_IDLE, __func__);
	} else {
		/* Retry Part. */
	...
	VOID *elem_obj)
{
	...
	if (Reason == MLME_SUCCESS) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - AUTH OK\n"));
#ifdef MAC_REPEATER_SUPPORT
...
#endif /* APCLI_CONNECTION_TRIAL */
		{
			...
			MlmeEnqueueWithWdev(pAd, ASSOC_FSM,
								...
								&AssocReq, 0, wdev, &Elem->priv_data);
		}

		cntl_fsm_state_transition(wdev, CliIdx, CNTL_WAIT_ASSOC, __func__);
		...
		/* Only Ndis802_11AuthModeShared try shared key first, Ndis802_11AuthModeAutoSwitch use open first */
		else if (IS_SECURITY_SHARED_WEP_Entry(&pStaCfg->wdev)
#ifdef WEPAUTO_OPEN_FIRST
#else
			|| IS_SECURITY_AUTOSWITCH_Entry(&pStaCfg->wdev)
#endif
		...
		MlmeEnqueueWithWdev(pAd, AUTH_FSM,
							...
							sizeof(MLME_AUTH_REQ_STRUCT),
							&AuthReq, 0, wdev, &Elem->priv_data);
		cntl_fsm_state_transition(wdev, CliIdx, CNTL_WAIT_AUTH2, __func__);
	}
}
	...
	VOID *elem_obj)
{
	...
	if (Reason == MLME_SUCCESS) {
		...
		if (IS_AKM_AUTOSWITCH(pStaCfg->wdev.SecConfig.AKMMap)
			...
			ASSERT(pEntry);
			if ((CliIdx == NON_REPT_ENTRY) && pEntry) {
	...
	VOID *elem_obj)
{
	...
	if (Reason == MLME_SUCCESS) {
		...
		link_up_type |= LINK_HAVE_INTER_SM_DATA;
...
#ifdef MAC_REPEATER_SUPPORT
		if (CliIdx != NON_REPT_ENTRY) {
			pEntry = pReptEntry ? &pAd->MacTab.Content[pReptEntry->MacTabWCID] : NULL;
		}
#endif
		if (pEntry)
			MacTableDeleteEntry(pAd, pEntry->wcid, pEntry->Addr);
	...
	if (Result == MLME_SUCCESS) {
		...
		if (pStaCfg->Dot11RCommInfo.bFtSupport &&
			...
			MlmeEnqueueWithWdev(pAd, MLME_CNTL_STATE_MACHINE,
								...
								(VOID *) &ApSsid, 0, wdev, &Elem->priv_data);
			cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_IDLE, __func__);
		} else
#endif /* DOT11R_FT_SUPPORT */
		{
	...
	VOID *elem_obj)
{
	...
	REPEATER_CLIENT_ENTRY *elemReptEntry;
	UCHAR elemCliidx;
#endif
	Elem = (MLME_QUEUE_ELEM *)elem_obj;
	wdev = Elem->wdev;
	pAd = (RTMP_ADAPTER *)wdev->sys_handle;
	CNTL_GET_STA_CFG(pAd, wdev, pStaCfg);
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - DeAuth successful\n"));
	...
	if (CliIdx != NON_REPT_ENTRY) {
		REPEATER_CLIENT_ENTRY *pReptCliEntry = Elem->priv_data.rept_cli_entry;

		ASSERT(pReptCliEntry);
		bValid = pReptCliEntry->CliValid;
		pLinkDownReason = &pReptCliEntry->LinkDownReason;
	}

	/*check the rept entry which is linked to the CliLink should be disabled, too.*/
	if (CliIdx == NON_REPT_ENTRY) {
		elemReptEntry = Elem->priv_data.rept_cli_entry;
		elemCliidx = Elem->priv_data.rept_cli_idx;
		if (pAd->ApCfg.bMACRepeaterEn == TRUE) {
			for (index = 0; index < GET_MAX_REPEATER_ENTRY_NUM(cap); index++) {
				pReptEntry = &pAd->ApCfg.pRepeaterCliPool[index];
				if ((pReptEntry->CliEnable) && (pReptEntry->wdev == &pStaCfg->wdev)) {
					if (pReptEntry->CliValid) {
						...
						Elem->priv_data.rept_cli_entry = pReptEntry;
						Elem->priv_data.rept_cli_idx = pReptEntry->MatchLinkIdx;
		...
		Elem->priv_data.rept_cli_entry = elemReptEntry;
		Elem->priv_data.rept_cli_idx = elemCliidx;
	}
	...
	if (Elem->priv_data.rept_cli_idx != NON_REPT_ENTRY) {
		cntl_fsm_state_transition(wdev, CliIdx, CNTL_IDLE, __func__);
		return;
	} else
#endif /* MAC_REPEATER_SUPPORT */
	...
	VOID *elem_obj)
{
	...
	pAd = (RTMP_ADAPTER *)wdev->sys_handle;
	CNTL_GET_STA_CFG(pAd, wdev, pStaCfg);
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - Dis-associate successful\n"));
	RTMPSendWirelessEvent(pAd, IW_DISASSOC_EVENT_FLAG, NULL, BSS0, 0);
	link_down_type |= LINK_HAVE_INTER_SM_DATA;
	bValid = pStaCfg->ApcliInfStat.Valid;
	pLinkDownReason = &pStaCfg->ApcliInfStat.LinkDownReason;
	...
	if (Elem->priv_data.rept_cli_idx != NON_REPT_ENTRY) {
		cntl_fsm_state_transition(wdev, CliIdx, CNTL_IDLE, __func__);  /* don't try other BSS for Rept, simply restore state to CNTL_IDLE */
		return;
	} else
#endif /* MAC_REPEATER_SUPPORT */
	{
	...
	VOID *elem_obj)
{
	...
	/*Reset the state machine to allow scan while Ap-Cli is Enable but not connected*/
	if (wdev->wdev_type == WDEV_TYPE_STA) {
		if (!cntl_idle(wdev)) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
				("(%s) sta idx [%d] Reset CNTL/SYNC/AUTH/ASSOC state machine !!!\n"
	...
	MlmeEnqueueWithWdev(pAd, SYNC_FSM, SYNC_FSM_SCAN_REQ,
						Elem->MsgLen, Elem->Msg, 0, wdev, &Elem->priv_data);
	cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_WAIT_SYNC, __func__);
...
VOID LinkUp_Infra(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, MAC_TABLE_ENTRY *pEntry, UCHAR *tmpWscSsid, UCHAR tmpWscSsidLen, UINT link_up_type, MLME_QUEUE_ELEM *Elem)
{
	...
	USHORT ifIndex = 0;
	BOOLEAN has_inter_sm_data = link_up_type & LINK_HAVE_INTER_SM_DATA;
	UCHAR CliIdx = (has_inter_sm_data) ? Elem->priv_data.rept_cli_idx : NON_REPT_ENTRY;
	...
	REPEATER_CLIENT_ENTRY *pReptEntry = (has_inter_sm_data) ? Elem->priv_data.rept_cli_entry : NULL;

	if (CliIdx != NON_REPT_ENTRY)
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s - !!! Rept CliIdx %d !!!\n", __func__, CliIdx));

#endif /* defined(MAC_REPEATER_SUPPORT)  */
	Cancelled = TRUE;
	...
	pStaCfg->PwrMgmt.ucBeaconPeriod = pStaCfg->MlmeAux.BeaconPeriod;
	pStaCfg->PwrMgmt.ucDtimPeriod = pStaCfg->MlmeAux.DtimPeriod;
	pStaCfg->PwrMgmt.bTriggerRoaming = FALSE;
	pStaCfg->PwrMgmt.ucWcid = pEntry->wcid;
#ifdef STA_LP_PHASE_1_SUPPORT
	RTMPClearEnterPsmNullBit(&pStaCfg->PwrMgmt);
#endif /* STA_LP_PHASE_1_SUPPORT */
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s::uBeaconPeriod(%d), uDtimPeriod(%d)\n", __func__, pStaCfg->PwrMgmt.ucBeaconPeriod, pStaCfg->PwrMgmt.ucDtimPeriod));
	...
	if (IF_COMBO_HAVE_AP_STA(pAd)) {
		...
		if (CliIdx != NON_REPT_ENTRY
			...
			pReptEntry->MacTabWCID = pEntry->wcid;
			pReptEntry->CliValid = TRUE;
			pEntry->bReptCli = TRUE;
			pEntry->bReptEthBridgeCli = FALSE;
			pEntry->MatchReptCliIdx = CliIdx;
			pEntry->ReptCliIdleCount = 0;
			COPY_MAC_ADDR(pEntry->ReptCliAddr, pReptEntry->CurrentAddress);
			tr_entry->OmacIdx = HcGetRepeaterOmac(pAd, pEntry);

			if (pReptEntry->bEthCli == TRUE) {
				pEntry->bReptEthCli = TRUE;

				if (MAC_ADDR_EQUAL(pAd->ApCfg.BridgeAddress, pAd->ApCfg.pRepeaterCliPool[CliIdx].OriginalAddress))
					pEntry->bReptEthBridgeCli = TRUE;
			} else
...
fill pEntry->seconfig before the 1st Eapol is received*/
...
#endif
		{
			...
			if (Bssidx != BSS_NOT_FOUND)
				...
				MTWF_LOG(DBG_CAT_CLIENT, CATCLIENT_APCLI, DBG_LVL_ERROR,
						 ("%s[%d] ERROR! can't find link up BSS, use config BW setting instead\n", __func__, __LINE__));
		...
		if ((CliIdx == NON_REPT_ENTRY) && (tr_entry->PortSecured == WPA_802_1X_PORT_SECURED)) {
	...
	if (CliIdx == 0xFF && wdev_do_linkup(wdev, pEntry) != TRUE)
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s(): linkup fail!!\n", __func__));
	...
	while (TRUE) {
		...
		RTMP_IndicateMediaState(pAd, NdisMediaStateDisconnected);
		pAd->ExtraInfo = GENERAL_LINK_DOWN;
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("NDIS_STATUS_MEDIA_DISCONNECT Event AA!\n"));
		break;
	}
...
#ifdef DOT11_VHT_AC
	MacTableEntryCheck2GVHT(pAd, pEntry, pStaCfg->MlmeAux.vht_cap_len);
#endif /* DOT11_VHT_AC */
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! LINK UP !!!  ClientStatusFlags=%lx)\n",
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! LINK UP !! (StaActive.bHtEnable =%d)\n",
	...
	if (CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_PIGGYBACK_CAPABLE)) {
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Turn on Piggy-Back\n"));
	}
...
#ifdef STA_LP_PHASE_1_SUPPORT
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Add TxSType(PID_NULL_FRAME_PWR_SAVE)\n"));
...
VOID LinkUp(RTMP_ADAPTER *pAd, UCHAR BssType, struct wifi_dev *wdev, UINT link_up_type, MLME_QUEUE_ELEM *Elem)
{
	...
	addht = wlan_operate_get_addht(wdev);
	tr_entry = &pAd->MacTab.tr_entry[pEntry->wcid];
	/*for rate adapter can refer wdev to get phy mode & channel*/
	pEntry->wdev = wdev;
	/*
	...
	*/
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("!!!%s LINK UP !!!\n", (BssType == BSS_ADHOC ? "ADHOC" : "Infra")));
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("!!! LINK UP !!! wdev %d, (BssType=%d, AID=%d, ssid=%s, Channel=%d, CentralChannel = %d)\n",
			  wdev->func_idx, BssType, pStaCfg->StaActive.Aid, pStaCfg->Ssid,
			  wdev->channel, wlan_operate_get_cen_ch_1(wdev)));
	NdisGetSystemUpTime(&Now);
	pStaCfg->LastBeaconRxTime = Now;	/* last RX timestamp */
#ifdef DOT11_N_SUPPORT
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! LINK UP !!! (Density =%d, )\n", pEntry->MpduDensity));
#endif /* DOT11_N_SUPPORT */
	...
	if (pStaCfg->StaActive.SupportedPhyInfo.bHtEnable == TRUE) {
		...
		if (ht_info2->OperaionMode >= _NOT_DEFINE_HT_PROTECT)
			wdev->protection |= SET_PROTECT(_NOT_DEFINE_HT_PROTECT);
		else
	...
	if (wdev->WscControl.WscConfMode == WSC_DISABLE) {
		if (LED_MODE(pAd) == WPS_LED_MODE_9) {	/* LED mode 9. */
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s: LedConnSecurityStatus(%d)\n",
					 __func__, led_status));
		}
	}
	...
	if (IS_HIF_TYPE(pAd, HIF_MT)) {
		...
		MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():Send HW_STA_BF_SOUNDING_ADJUST Cmd\n", __func__));
	...
	AsicSetTxStream(pAd, pAd->Antenna.field.TxPath, OPMODE_STA, TRUE, BandIdx);
	...
	if (INFRA_ON(pStaCfg)) {
		if ((pAd->CommonCfg.bBssCoexEnable == TRUE)
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("not supports 20/40 BSS COEX !!!\n"));
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
	...
	if (p2p_wdev
		...
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("p2p_wdev->channel %d wdev->channel %d\n", p2p_wdev->channel, wdev->channel));

		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {
			if ((op_ht_bw1 != op_ht_bw2) && ((wdev->channel == p2p_wdev->channel))) {
				MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("start bw !=  && P2P GO SCC\n"));
			...
			if (pMacEntry) {
				if (IS_ENTRY_PEER_AP(pMacEntry) && (pMacEntry->PairwiseKey.KeyLen == LEN_TK)) { /* P2P GC will have security */
					if ((op_ht_bw1 != op_ht_bw2) && ((wdev->channel == p2p_wdev->channel))) {
						MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("start bw !=  && P2P GC SCC\n"));
						pAd->Mlme.bStartScc = TRUE;
					}
...
INT LinkDown_Adhoc(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("!!! LINK DOWN 1!!!\n"));
	OPSTATUS_CLEAR_FLAG(pAd, fOP_STATUS_ADHOC_ON);
	STA_STATUS_CLEAR_FLAG(pStaCfg, fSTA_STATUS_MEDIA_STATE_CONNECTED);
	RTMP_IndicateMediaState(pAd, NdisMediaStateDisconnected);
	pAd->ExtraInfo = GENERAL_LINK_DOWN;
	BssTableDeleteEntry(ScanTab, pStaCfg->Bssid, wdev->channel);
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("MacTab.Size=%d\n", pAd->MacTab.Size));

	if (wdev_do_linkdown(wdev) != TRUE)
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s: linkdown fail!", __func__));
...
INT LinkDown_Infra(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, BOOLEAN ReqByAP, BOOLEAN has_inter_sm_data, MLME_QUEUE_ELEM *Elem)
{
	PSTA_ADMIN_CONFIG pStaCfg = GetStaCfgByWdev(pAd, wdev);
...
#endif
#ifdef MAC_REPEATER_SUPPORT
	UCHAR CliIdx = (has_inter_sm_data) ? Elem->priv_data.rept_cli_idx : 0xFF;
	/* REPEATER_CLIENT_ENTRY *pReptEntry = (has_inter_sm_data) ? Elem->priv_data.rept_cli_entry : NULL; */
#endif /* MAC_REPEATER_SUPPORT */
	USHORT ifIndex = wdev->func_idx;
	BSS_TABLE *ScanTab = get_scan_tab_by_wdev(pAd, wdev);
	...
	if (pStaCfg->MlmeAux.CurrReqIsFromNdis == TRUE) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("NDIS_STATUS_MEDIA_DISCONNECT Event A!\n"));
	...
	if (pAd->Mlme.bStartScc == TRUE) {
		...
		if (RTMP_CFG80211_VIF_P2P_GO_ON(pAd)) {
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("link down and switch to GO bw %d   pWdev->CentralChanne  %d\n", op_ht_bw, pWdev->CentralChannel));
...
#endif
#if defined(STA_LP_PHASE_1_SUPPORT) || defined(STA_LP_PHASE_2_SUPPORT)
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("Remove TxSType(PID_NULL_FRAME_PWR_SAVE)\n"));
...
#endif /* MAC_REPEATER_SUPPORT */
			{
				...
				if (wf_drv_tbl.wf_fwd_check_device_hook)
					wf_drv_tbl.wf_fwd_check_device_hook(pStaCfg->wdev.if_dev,
									INT_APCLI, CliIdx, pStaCfg->wdev.channel, 0);
...
VOID LinkDown(RTMP_ADAPTER *pAd, UINT linkdown_type, struct wifi_dev *wdev, MLME_QUEUE_ELEM *Elem)
{
	...
	BOOLEAN has_inter_sm_data = linkdown_type & LINK_HAVE_INTER_SM_DATA;
#ifdef MAC_REPEATER_SUPPORT
	REPEATER_CLIENT_ENTRY *pReptEntry = (has_inter_sm_data) ? Elem->priv_data.rept_cli_entry : NULL;
#endif /* MAC_REPEATER_SUPPORT */
	UCHAR CliIdx = (has_inter_sm_data) ? Elem->priv_data.rept_cli_idx : 0xFF;
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s] wdev %d pEntry=%p addr=%02X:%02X:%02X:%02X:%02X:%02X\n",
			 __func__, wdev->func_idx, (PVOID)pEntry, PRINT_MAC(pEntry->Addr)));
#ifdef MT_WOW_SUPPORT

	if (pAd->WOW_Cfg.bWoWRunning) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("[%s] WoW is running, skip!\n", __func__));
		return;
	}
...
#endif /* CONFIG_STA_ADHOC_SUPPORT */
		LinkDown_Infra(pAd, wdev, req_from_ap, has_inter_sm_data, Elem);
	...
	/* Check whether disable/enable BF souding timer for STA mode only */
	IF_DEV_CONFIG_OPMODE_ON_STA(pAd) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s():Send HW_STA_BF_SOUNDING_ADJUST Cmd\n", __func__));
...
#endif /* MAC_REPEATER_SUPPORT */
    {
	if (VALID_UCAST_ENTRY_WCID(pAd, pStaCfg->MacTabWCID))
	...
	TRTableResetEntry(pAd, wdev->bss_info_argument.ucBcMcWlanIdx);
	MacTableDeleteEntry(pAd, pEntry->wcid, pEntry->Addr);
	...
	if (ADHOC_ON(pAd)) {
		...
		if (CliIdx == 0xFF && wdev_do_linkdown(wdev) != TRUE)
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("LinkDownCleanMlmeAux.ExtCapInfo!\n"));
	...
	pAd->MacTab.tr_entry[pEntry->wcid].PortSecured = wdev->PortSecured;
	NdisReleaseSpinLock(&pAd->MacTabLock);
	pStaCfg->MicErrCnt = 0;
	RTMP_IndicateMediaState(pAd, NdisMediaStateDisconnected);
...
VOID IterateOnBssTab(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (pStaCfg->BssType == BSS_ADHOC) {
...

		{
			...
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("CNTL - All BSS fail; start a new ADHOC (Ssid=%s)...\n", pStaCfg->MlmeAux.Ssid));
			StartParmFill(pAd, &StartReq, (PCHAR) pStaCfg->MlmeAux.Ssid, pStaCfg->MlmeAux.SsidLen);
			MlmeEnqueueWithWdev(pAd, SYNC_FSM, SYNC_FSM_ADHOC_START_REQ, sizeof(MLME_START_REQ_STRUCT), &StartReq, 0, wdev, NULL);
			cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_WAIT_SYNC, __func__);
		}
	} else
#endif /* CONFIG_STA_ADHOC_SUPPORT */
...
VOID IterateOnBssTab2(RTMP_ADAPTER *pAd, struct wifi_dev *wdev)
{
	...
	if (BssIdx < pStaCfg->MlmeAux.RoamTab.BssNr) {
		...
		MlmeEnqueueWithWdev(pAd, ASSOC_FSM, ASSOC_FSM_MLME_REASSOC_REQ,
							sizeof(MLME_REASSOC_REQ_STRUCT), &ReassocReq, 0, wdev, NULL);
		cntl_fsm_state_transition(wdev, NON_REPT_ENTRY, CNTL_WAIT_ASSOC, __func__);
	} else {
		/* no more BSS */
	...
	IN struct wifi_dev *wdev)
{
	...
	*pBwFallBack = 0;
	...
	MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s(): RF-Ch=%d, CtrlCh=%d, CentralCh=%d, CentralCh2=%d, BW=%d\n",
			  __func__,
