	...
	IN USHORT len)
{
	...
	for	(i = 1;	i <= ((len_in_bits + 383) / 384); i++) {
		...
		RT_HMAC_SHA384((UCHAR *)key, key_len, input, total_len, (UCHAR *)&output[currentindex], SHA384_DIGEST_SIZE);
	...
	IN PMAC_TABLE_ENTRY pEntry)
{
	...
	case WDEV_TYPE_STA:
		...
		MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("==> %s(STA)\n", __func__));
		break;
#endif /* CONFIG_STA_SUPPORT */
	}
	...
	OUT PEAPOL_PACKET pMsg)
{
	...
	if (IS_FT_RSN_STA(pEntry) && bWPA2 &&
		...
		PRTMP_ADAPTER pAd = (PRTMP_ADAPTER)pEntry->wdev->sys_handle;
#ifdef CONFIG_AP_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd) {
	...
	OUT PEAPOL_PACKET pMsg)
{
	...
	if ((pMsg->KeyDesc.KeyInfo.KeyMic == 1) &&
	   ((IS_AKM_SHA384(pSecPairwise->AKMMap)) || (pSecPairwise->key_deri_alg == SEC_KEY_DERI_SHA384)))
	...
	while (len > sizeof(RSNIE3)) {
		...
		if (pEid->Eid == IE_RSN) {
			if ((bWPA2) &&
				...
				if (NdisEqualMemory(pVIE, rsn_ie, rsn_ie[1] + 2)) {
					MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s ==> RSN IE matched, Length(%d)\n",
										__func__, (pEid->Len + 2)));
					result = TRUE;
				}
#ifdef DOT11R_FT_SUPPORT
					...
					if (IS_AKM_FT_WPA2PSK(pSecConfig->AKMMap) ||
						...
						/* group cipher*/
						pRecSuite = WPA_ExtractSuiteFromRSNIE(pVIE, (pEid->Len + 2), GROUP_SUITE, &RecCount);
						...
						if ((RecCount == ConCount) && (pRecSuite) && (pConSuite) &&
							...
							MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
								("%s ==> GROUP_SUITE mis-matched under FT\n",	__func__));
							CheckRes = FALSE;
						}

						/* PAIRWISE_SUITE */
						pRecSuite = WPA_ExtractSuiteFromRSNIE(pVIE, (pEid->Len + 2), PAIRWISE_SUITE, &RecCount);
						...
						if ((RecCount == ConCount) && (pRecSuite) && (pConSuite) &&
							...
							if (pConSuite)
								hex_dump("ConIE", pConSuite, (4 * ConCount));
							MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
								("%s ==> PAIRWISE_SUITE mis-matched under FT\n",	__func__));
							CheckRes = FALSE;
						}

						/* AKM_SUITE */
						pRecSuite = WPA_ExtractSuiteFromRSNIE(pVIE, (pEid->Len + 2), AKM_SUITE, &RecCount);
						...
						if ((RecCount == ConCount) && (pRecSuite) && (pConSuite) &&
							...
							if (pConSuite)
								hex_dump("ConIE", pConSuite, (4 * ConCount));
							MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
								("%s ==> AKM_SUITE mis-matched under FT\n",	__func__));
							CheckRes = FALSE;
						}


						/* RSN_CAP_INFO */
						pRecSuite = WPA_ExtractSuiteFromRSNIE(pVIE, (pEid->Len + 2), RSN_CAP_INFO, &RecCount);
						...
						if ((RecCount == ConCount) && (pRecSuite) && (pConSuite) &&
							...
							MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
								("%s ==> RSN_CAP_INFO mis-matched under FT\n",	__func__));
							CheckRes = FALSE;
						}
				...
				MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> RSN IE Lens mis-matched\n", __func__));
			...
			*Offset += (pEid->Len + 2);
			...
			if ((CLIENT_STATUS_TEST_FLAG(pEntry, fCLIENT_STATUS_OSEN_CAPABLE) &&
				...
				MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s ==> HS2.0 OSEN IE matched, Length(%d)\n",
										__func__, (pEid->Len + 2)));
				result = TRUE;

			} else
				MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> HS2.0 OSEN IE mis-matched\n", __func__));
		...
		/* WPA IE*/
			...
			if ((!bWPA2) &&
				...
				(wpa_ie && NdisEqualMemory(pVIE, wpa_ie, wpa_ie[1] + 2))) {
					MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s ==> WPA/WPAPSK IE matched, Length(%d)\n",
											__func__, (pEid->Len + 2)));
					result = TRUE;

			} else {
				MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s ==> WPA/WPAPSK IE mis-matched\n", __func__));
			}
	...
	IN MAC_TABLE_ENTRY * pEntry)
{
	...
	if (!RTMPCheckRSNIE(pAd, pKeyData, KeyDataLen, bWPA2, pEntry, &skip_offset)) {
			...
			MTWF_LOG(DBG_CAT_SEC, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("RSN_IE Different in msg %d of 4-way handshake!\n", MsgType));
...
/* Cache the PMK against PMKID, generated in process_ecdh_element() */
	...
	if (IS_AKM_OWE(pSecConfig->AKMMap)) {
		...
		if (owe->pmkid) {
			sta_add_pmkid_cache(pAd, pEntry->Addr, owe->pmkid, pSecConfig->PMK, pmk_len, pStaCfg->wdev.func_idx, CliIdx);
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (WpaMessageSanity(pAd, pReceiveEapolM4, EapolLen, EAPOL_PAIR_MSG_4, pSecConfig, pEntry, pSecConfig->PTK) == FALSE)
		return;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (WpaMessageSanity(pAd, pReceiveEapol, EapolLen, EAPOL_GROUP_MSG_1, pSecConfig, pEntry, pSecConfig->PTK) == FALSE)
		return;
	...
	IN MLME_QUEUE_ELEM * Elem)
{
	...
	if (WpaMessageSanity(pAd, pReceiveEapolM2, EapolLen, EAPOL_GROUP_MSG_2, pSecConfig, pEntry, pSecConfig->PTK) == FALSE)
		return;
	...
	IN PVOID SystemSpecific3)
{
	...
	if (IS_AKM_PSK(pSecConfig->AKMMap)
		...
		MlmeEnqueueWithWdev(pAd, WPA_STATE_MACHINE, MT2_EAPOLStart, 6, &pEntry->Addr, 0, pEntry->wdev, NULL);
