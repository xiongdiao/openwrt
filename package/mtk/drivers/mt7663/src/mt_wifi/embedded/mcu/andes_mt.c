...
#if defined(MT7663)
	if (IS_MT7663(pAd))
		dfs_rdd_pulsedata(pAd, pExtEventRddReport);
#endif
	...
	UINT16 rsp_payload_len)
{
	...
	for (i = 0; i < DBDC_BAND_NUM; i++) {
		...
		if (i == ENUM_BAND_0) {
			pAd->MsMibBucket.PdCount[i][CurrIdx] = pData->rScsPhyInfo[i].u4CckPdCnt;
			pAd->MsMibBucket.MdrdyCount[i][CurrIdx] = pData->rScsPhyInfo[i].u4CckMdrdyErrCnt;
		} else {
			pAd->MsMibBucket.PdCount[i][CurrIdx] = pData->rScsPhyInfo[i].u4OfdmPdCnt;
			pAd->MsMibBucket.MdrdyCount[i][CurrIdx] = pData->rScsPhyInfo[i].u4OfdmMdrdyErrCnt;
		}
	}
}
