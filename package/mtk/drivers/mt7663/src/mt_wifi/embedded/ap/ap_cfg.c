...
static struct {
	...
	*/
...
#ifdef APCLI_SUPPORT
#ifdef UAPSD_SUPPORT
	{"ApCliAPSDCapable",		set_ApCli_UAPSD_Proc},
	...
	{"set_muru_algo_ctrl",				SetMuruAlgoCtrl},
	...
	{"vow_rx_airtime_show_en", set_vow_rx_airtime_show_en},
	...
	/* wtbl airtime mode */
	{"vow_wtbl_adm_mode", set_vow_wtbl_adm_mode},
	...
	*/
	...
	{"ThermalItemInfo",     SetThermalItemInfo},
...
#ifdef CFG_SUPPORT_FALCON_MURU
	{"get_muru_algo_ctrl",				ShowMuruAlgoCtrl},
	...
	IN INT cmd)
{
	...
	case OID_802_11_CURRENT_CHANNEL_INFO:
	{
		...
		bandidx = HcGetBandByChannel(pAd, Rsp.channel);
				...
				*/
				if (pAd->ApCfg.bMACRepeaterEn) {
					...
					if (pReptEntry) {
						...
						priv_data.rept_cli_idx = pEntry->MatchReptCliIdx;
	...
	case OID_SET_SSID:
		if (wrq->u.data.length <= MAX_LEN_OF_SSID) {
			...
			if (pObj->ioctl_if_type == INT_APCLI) {
				apcli_entry = &pAd->StaCfg[apcli_idx];
	...
	case OID_802_11_OCE_REDUCED_NEIGHBOR_REPORT: {/*pure 1x is enabled. */
		...
		if (IS_OCE_RNR_ENABLE(wdev)) {
			pAd->ApCfg.MBSSID[apidx].ReducedNRListExist  = TRUE;
	...
	case OID_APCLI_VENDOR_IE_SET:
	{
		...
		if (ret == NDIS_STATUS_SUCCESS) {
			...
			Status = copy_from_user(vendor_ie_temp,
						wrq->u.data.pointer,
						wrq->u.data.length);
			apcli_vendor_ie = &pAd->StaCfg[apidx].apcli_vendor_ie;
	...
	IN INT cmd)
{
	...
	case OID_GET_AIR_MONITOR_RESULT:
		{
			...
			for (i = 0; i < MAX_NUM_OF_MONITOR_STA; i++) {
				...
				if (pTmpEntry->bValid) {
					...
					pEntryLen += sizeof(MNT_STA_ENTRY);
				}
			}
	...
	case OID_GET_CPU_TEMPERATURE:{
		...
		ucBand = HcGetBandByWdev(&pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev);
	...
	case RT_QUERY_11H_CAPABILITY:
		{
			...
			if (!pEntry || !(IS_AKM_OPEN(pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.SecConfig.AKMMap) ||
				((pEntry->SecConfig.Handshake.WpaState == AS_PTKINITDONE) &&
				(pEntry->SecConfig.Handshake.GTKState == REKEY_ESTABLISHED)))) {
				Status = -ENOTCONN;
				MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
	...
	case RT_QUERY_WNM_CAPABILITY: {
		...
		if (!pEntry ||
			...
			(pEntry->SecConfig.Handshake.GTKState == REKEY_ESTABLISHED)))) {
				MTWF_LOG(DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_ERROR,
					("RT_QUERY_WNM_CAPABILITY: STA(%02x:%02x:%02x:%02x:%02x:%02x)"
					"not associates with AP!\n", PRINT_MAC(p_wnm_query_data)));
				Status = EINVAL;
				os_free_mem(p_wnm_command);
				break;
		}
	...
	case RT_QUERY_RRM_CAPABILITY: {
		...
		if (!pEntry ||
			...
			(pEntry->SecConfig.Handshake.GTKState == REKEY_ESTABLISHED)))) {
			MTWF_LOG(DBG_CAT_PROTO, CATPROTO_RRM, DBG_LVL_ERROR,
				("RT_QUERY_RRM_CAPABILITY: STA(%02x:%02x:%02x:%02x:%02x:%02x)"
				"not associates with AP!\n",
	...
	IN	RTMP_STRING *arg)
{
	...
	MAC_IO_READ32(pAd->hdev_ctrl, addr, &vlantag);
	vlantag = DMA_VTR_SET_VID(wdev->OmacIdx, vlantag, vid);
	MAC_IO_WRITE32(pAd->hdev_ctrl, addr, vlantag);
...
INT Show_Radio_Stat_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	printk("PER(percent from start)	= %u\n", ctrl->rdev[0].pRadioCtrl->TotalPER);
...
INT Show_TxBfInfo_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
    ...
    HT_BF_CAP *pTxBFCap;
	PMAC_TABLE_ENTRY pEntry = NULL;
	BOOLEAN eTxBfCap = FALSE, SU_MU = FALSE;
	UINT8 txMode = 0, bw = 0, BFeeMaxNr = 0, Nc = 0;
	UINT8 peerRxNumSupport = 0, txMCSSetdefined = 0;
	UINT8 txrxMCSSetNotEqual = 0, txMaxNumSpatilStream = 0;
	...
	if (cap->FlgHwTxBfCap) {
		...
		if (VALID_UCAST_ENTRY_WCID(pAd, wcid)) {
			...
			if (pEntry->EntryType == ENTRY_CAT_MCAST)
				return FALSE;

			switch (pEntry->MaxHTPhyMode.field.MODE) {
#ifdef DOT11_VHT_AC
			case MODE_VHT:
				eTxBfCap = mt_WrapClientSupportsVhtETxBF(pAd, &pEntry->vht_cap_ie.vht_cap);
				txMode = 4; /* VHT mode */
				bw = pEntry->MaxHTPhyMode.field.BW;
				SU_MU = pEntry->vht_cap_ie.vht_cap.bfee_cap_mu;
#endif
				BFeeMaxNr = pEntry->vht_cap_ie.vht_cap.bfee_sts_cap;
				peerRxNumSupport = (pEntry->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss2 != 3) ? 1 : 0;
				peerRxNumSupport = (pEntry->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss3 != 3) ? 2 : peerRxNumSupport;
				peerRxNumSupport = (pEntry->vht_cap_ie.mcs_set.rx_mcs_map.mcs_ss4 != 3) ? 3 : peerRxNumSupport;
				Nc = peerRxNumSupport;
				break;

			case MODE_HTMIX:
			case MODE_HTGREENFIELD:
				eTxBfCap = mt_WrapClientSupportsETxBF(pAd, &pEntry->HTCapability.TxBFCap);
				txMode = 2;
				bw = pEntry->MaxHTPhyMode.field.BW;
				BFeeMaxNr = pEntry->HTCapability.TxBFCap.ComSteerBFAntSup;
				txMCSSetdefined = ((pEntry->HTCapability.MCSSet[12] &
							TX_MCS_SET_DEFINED) >> TX_MCS_SET_DEFINED_OFFSET);
				txrxMCSSetNotEqual = ((pEntry->HTCapability.MCSSet[12] &
							TX_RX_MCS_SET_N_EQUAL) >> TX_RX_MCS_SET_N_EQUAL_OFFSET);
				txMaxNumSpatilStream = ((pEntry->HTCapability.MCSSet[12] &
							TX_MAX_NUM_SPATIAL_STREAMS_SUPPORTED)
						>> TX_MAX_NUM_SPATIAL_STREAMS_SUPPORTED_OFFSET);
				peerRxNumSupport = (pEntry->HTCapability.MCSSet[1] > 0) ? 1 : 0;
				peerRxNumSupport = (pEntry->HTCapability.MCSSet[2] > 0) ? 2 : peerRxNumSupport;
				peerRxNumSupport = (pEntry->HTCapability.MCSSet[3] > 0) ? 3 : peerRxNumSupport;
				Nc = peerRxNumSupport;
				if ((txMCSSetdefined == 1) && (txrxMCSSetNotEqual == 1))
					Nc = txMaxNumSpatilStream;
				break;
			...
			MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("STA\n"));
	...
	IN	RTMP_IOCTL_INPUT_STRUCT	 *wrq)
{
	...
	USHORT				eepValue, string_size, new_line_size;
...
VOID RTMPIoctlStatistics(RTMP_ADAPTER *pAd, RTMP_IOCTL_INPUT_STRUCT *wrq)
{
	...
	if (ATE_ON(pAd)) {
		...
		per = pAd->WlanCounters[ucBand].ReceivedFragmentCount.u.LowPart == 0 ? 0 : 1000 *
			  (pAd->WlanCounters[ucBand].FCSErrorCount.u.LowPart) / (pAd->WlanCounters[ucBand].FCSErrorCount.u.LowPart +
					  pAd->WlanCounters[ucBand].ReceivedFragmentCount.u.LowPart);
...

	{
		...
		sprintf(msg + strlen(msg), "GAS MMPDU size                            = %d\n", pGASCtrl->MMPDUSize);
	...
	IN	RTMP_STRING *arg)
{
	...
	apcli_entry = &pAd->StaCfg[pObj->ioctl_if];
	wdev = &apcli_entry->wdev;
	rate = &wdev->rate;
	...
	IN	RTMP_STRING *arg)
{
	...
	if (strlen(arg) ==	(UUID_LEN_STR - 1)) {
		...
		pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.WscControl.Wsc_Uuid_Str[UUID_LEN_STR - 1] = 0;
		NdisMoveMemory(&pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.WscControl.Wsc_Uuid_Str[0], arg, strlen(arg));
		return TRUE;
	} else {
	...
	IN	RTMP_STRING *arg)
{
	...
	if (strlen(arg) ==	(UUID_LEN_HEX * 2)) {
		...
		AtoH(arg, &pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.WscControl.Wsc_Uuid_E[0], UUID_LEN_HEX);
		hex_dump("Set_WscUUID_HEX_E_Proc OK:",
			&pAd->ApCfg.MBSSID[pObj->ioctl_if].wdev.WscControl.Wsc_Uuid_E[0], UUID_LEN_HEX);
		return TRUE;
	} else {
...
INT	Set_AP_TimEnable_Proc(RTMP_ADAPTER *pAd, RTMP_STRING *arg)
{
	...
	if (interval > 0) { /* valid interval --> enable */
		fgEnable = TRUE;
		MAC_IO_WRITE32(pAd->hdev_ctrl, LPON_PTTISR, IDX_DEFAULT_PRETTTT_INTERVAL(BssidIdxMapped));
		MAC_IO_READ32(pAd->hdev_ctrl, u4TtttEnCr, &Value);
		Value &= ~TTTTn_OFFSET_OF_TBTTn_MASK;
		Value |= DEFAULT_TTTT_OFFSET_IN_MS;
		Value |= TTTTn_CAL_EN;
		MAC_IO_WRITE32(pAd->hdev_ctrl, u4TtttEnCr, Value);
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("u4TtttEnCr = %x, Value = %x\n", u4TtttEnCr, Value));
		MAC_IO_WRITE32(pAd->hdev_ctrl, u4TtttIntervelCr, SET_TTTT_PERIOD(pAd, interval));
		MTWF_LOG(DBG_CAT_CFG, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("u4TtttIntervelCr = %x, Value = %x\n", u4TtttIntervelCr,
				 SET_TTTT_PERIOD(pAd, interval)));
		MAC_IO_READ32(pAd->hdev_ctrl, LPON_MPTCR4, &Value);
		Value |= TTTT0_PERIODTIMER_EN |
				 TTTT0_TIMEUP_EN |
				 PRETTTT0TIMEUP_EN |
				 PRETTTT0_TRIG_EN |
				 PRETTTT0INT_EN;
		MAC_IO_WRITE32(pAd->hdev_ctrl, LPON_MPTCR4, Value);
		...
		MAC_IO_WRITE32(pAd->hdev_ctrl, AGG_BTIMRR0, Value);
		/*TODO: other omac idx rate setting.*/
		MAC_IO_READ32(pAd->hdev_ctrl, ARB_BTIMCR0, &Value);
		Value |= (1 << OmacIdx);
		MAC_IO_WRITE32(pAd->hdev_ctrl, ARB_BTIMCR0, Value);/* enable BTIM_EN bit in ARB_BTIMCR0 */
		MAC_IO_WRITE32(pAd->hdev_ctrl, ARB_BTIMCR1, Value);/* enable 2 TIM broadcast */
#ifndef BCN_OFFLOAD_SUPPORT
		MAC_IO_WRITE32(pAd->hdev_ctrl, HWIER4, 0x80008000);/* enable TTTT0/PRETTTT0 interrupt. */
#endif
	} else { /* interval 0 --> disable */
		MAC_IO_READ32(pAd->hdev_ctrl, ARB_BTIMCR0, &Value);
		Value &= ~(1 << OmacIdx);
		MAC_IO_WRITE32(pAd->hdev_ctrl, ARB_BTIMCR0, Value);
		MAC_IO_WRITE32(pAd->hdev_ctrl, ARB_BTIMCR1, Value);
	}
	...
	IN UINT32 ANQPRspLen)
{
	...
	if ((ANQPRspLen > pGASCtrl->MMPDUSize) ||
