...
INT32 SetATEDaByWtblTlv(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	UCHAR *Addr1 = &ATECtrl->Addr1[0];
#endif
#ifdef CONFIG_STA_SUPPORT
	UCHAR *Addr3 = &ATECtrl->Addr3[0];
#endif
#ifdef DBDC_MODE
	...
	for (Octet = 0, Value = rstrtok(Arg, ":"); Value; Value = rstrtok(NULL, ":")) {
		...
		IF_DEV_CONFIG_OPMODE_ON_AP(pAd)
			AtoH(Value, &Addr1[Octet++], 1);
#endif /* CONFIG_AP_SUPPORT */
#ifdef CONFIG_STA_SUPPORT
		IF_DEV_CONFIG_OPMODE_ON_STA(pAd)
			AtoH(Value, &Addr3[Octet++], 1);
	...
	NdisMoveMemory(rWtblGeneric.aucPeerAddress, Addr3, MAC_ADDR_LEN);
...
INT32 SetATESa(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	wdev_idx = ATECtrl->wdev_idx;
	wdev = pAd->wdev_list[ATECtrl->wdev_idx];
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: Sa = %s,wdev_idx=%d\n", __func__, Arg, wdev_idx));
	...
	AsicDevInfoUpdate(
		pAd,
		IS_MT7626(pAd) ? HcGetOmacIdx(pAd, wdev):0x0,
		Addr3,
		BandIdx,
		TRUE,
...
INT32 SetATEBssid(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: (BSSID = %02x:%02x:%02x:%02x:%02x:%02x)\n", __func__,
			  Addr1[0], Addr1[1], Addr1[2], ATECtrl->Addr1[3], Addr1[4], Addr1[5]));
#endif /* CONFIG_STA_SUPPORT */
	return TRUE;
}
...
INT32 GetATETxFreqOffset(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	FreqOffsetRxv = rx_stat_rxv->FreqOffsetFromRx;

	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("%s: FreqOffset = %d\n", __FUNCTION__, FreqOffsetRxv));
...
INT32 SetATEShow(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("isConcurrentTx = %d\n", ATECtrl->isConcurrentTx));
	MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF, ("isConcurrentRx = %d\n", ATECtrl->isConcurrentRx));
#endif
#endif /* DBDC_MODE */
...
INT32 SetATEIBfProfileUpdate(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	UCHAR control_band_idx = ATECtrl->control_band_idx;
	INT32   i;
	UCHAR   Nr, Nc, PfmuIdx, NdpNss, *value, ucBuf[3], aucPfmuMemRow[8], aucPfmuMemCol[8];
	RTMP_STRING cmdStr[80];
	...
	snprintf(cmdStr, 24, "%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x", aucPfmuMemCol[0], aucPfmuMemRow[0],
	...
	snprintf(cmdStr, sizeof(cmdStr), "01:00:%.2x:00:00:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
					PfmuIdx, NdpNss, Nc, Nr,
...
INT32 SetATEEBfProfileConfig(RTMP_ADAPTER *pAd, RTMP_STRING *Arg)
{
	...
	snprintf(cmdStr, sizeof(cmdStr), "01:00:%.2x:00:01:00:%.2x:00:02:%.2x:%.2x:00:00:00:00:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x:%.2x",
					PfmuIdx, NdpNss, Nc, Nr,
					aucPfmuMemRow[0], aucPfmuMemCol[0],
					aucPfmuMemRow[1], aucPfmuMemCol[1],
					aucPfmuMemRow[2], aucPfmuMemCol[2],
	...
	RTMP_STRING *Arg)
{
	...
	/* RtmpOsMsDelay(100); */
	/* Set Tx BW */
	snprintf(cmdStr, sizeof(cmdStr), "%d:%d", BW, BW);
...
#ifdef ATE_TXTHREAD
#if defined(ATE_CONCURRENT_SUPPORT)
	ATECtrl->isConcurrentTx = 0;
	ATECtrl->isConcurrentRx = 0;
#endif
#endif
	return Ret;
}
	...
	RTMP_STRING *Arg)
{
	...
	if (!strcmp(Arg, "ATESTART") && (mode != ATE_START)) { /* support restart w/o ATESTOP */
	...
	else if (!strcmp(Arg, "7663PREKClean") && (mode & ATE_START)) {
		Ret = ATEOp->PreCal7663(pAd, "0");
		Ret = 0;
	} else if (!strcmp(Arg, "7663PREK") && (mode & ATE_START)) {
		Ret = ATEOp->PreCal7663(pAd, "1");
		Ret = 0;
	} else if (!strcmp(Arg, "7663DPDClean") && (mode & ATE_START)) {
		Ret = ATEOp->TxDPD7663(pAd, "0");
		Ret = 0;
	} else if (!strcmp(Arg, "7663DPD5G") && (mode & ATE_START)) {
		Ret = ATEOp->TxDPD7663(pAd, "1");
		Ret = 0;
	} else if (!strcmp(Arg, "7663DPD2G") && (mode & ATE_START)) {
		Ret = ATEOp->TxDPD7663(pAd, "2");
		Ret = 0;
	}
		...
		if (mode & fATE_EXIT) {
			...
			ATECtrl->band_ext[0].wmm_idx = 1;
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%s: (param = (%s), mode = (%d)), ATECtrl->use_apcli = %d\n",
					  __func__, Arg, ATECtrl->Mode, ATECtrl->use_apcli));
		} else {
			MTWF_LOG(DBG_CAT_TEST, DBG_SUBCAT_ALL, DBG_LVL_OFF,
					 ("%s: [Warning](param = (%s), mode = (%d)), ATECtrl->use_apcli = %d\n",
					  __func__, Arg, ATECtrl->Mode, ATECtrl->use_apcli));
		}
