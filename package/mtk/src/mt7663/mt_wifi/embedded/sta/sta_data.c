	...
	PNDIS_PACKET pPacket)
{
	...
	if ((!pSrcBufVA) || (SrcBufLen <= 14)) {
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():pkt error(%p, %d)\n",
				 __func__, pSrcBufVA, SrcBufLen));
		return FALSE;
	}
	...
	if (!RTMPCheckEtherType(pAd, pPacket, &pAd->MacTab.tr_entry[wcid], wdev))
		return FALSE;
	...
	IN PUCHAR FreeNumberIs)
{
	...
	case QID_CTRL:
		if (hif->ctrl_ring.TxSwFreeIdx > hif->ctrl_ring.TxCpuIdx)
			FreeNumber = hif->ctrl_ring.TxSwFreeIdx - hif->ctrl_ring.TxCpuIdx - 1;
		else
			FreeNumber = hif->ctrl_ring.TxSwFreeIdx + CTL_RING_SIZE - hif->ctrl_ring.TxCpuIdx - 1;
...
INT sta_ieee_802_11_data_rx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
		...
		if ((pFmeCtrl->Wep == 1) && (pRxInfo->Decrypted == 0)) {
...
#endif /* HDR_TRANS_SUPPORT */
			{
				...
				if (IS_ENTRY_PEER_AP(pEntry)) {
					...
					if (!pSwKey) {
						MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("No vaild cipher key for SW decryption!!!\n"));
	...
	MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s():pRxInfo->Bcast =%d, pRxInfo->Mcast=%d, pRxBlk->Flags=0x%x, fRX_AP/STA/ADHOC=0x%x/0x%x/0x%x\n",
	...
	if (pRxInfo->Bcast || pRxInfo->Mcast) {
		...
		if (pFmeCtrl->MoreFrag) {
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): MoreFrag!\n", __func__));
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			return TRUE;
		}
		...
		if (pFmeCtrl->FrDs
			...
			MTWF_LOG(DBG_CAT_RX, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s(): pFmeCtrl->FrDs!\n", __func__));
			RELEASE_NDIS_PACKET(pAd, pRxBlk->pRxPacket, NDIS_STATUS_FAILURE);
			return TRUE;
		}
	...
	if (pRxInfo->U2M) {
		Update_Rssi_Sample(pAd, &pAd->StaCfg[0].RssiSample, &pRxBlk->rx_signal, pRxBlk->rx_rate.field.MODE, pRxBlk->rx_rate.field.BW);
...
INT sta_frag_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *pTxBlk)
{
	...
	if (IS_CIPHER_TKIP(pTxBlk->CipherAlg)) {
		RTMPCalculateMICValue(pAd, pTxBlk->pPacket, pTxBlk->pExtraLlcSnapEncap, pTxBlk->pKey, &pTxBlk->pKey[LEN_TK], 0);
		/*
	...
	/*MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("!!!Fragment AckDuration(%d), TxRate(%d)!!!\n", AckDuration, pTxBlk->TxRate)); */
#ifdef SOFT_ENCRYPT
	...
	if (TX_BLK_TEST_FLAG(pTxBlk, fTX_bSwEncrypt)) {
		...
		if (tmp_ptr == NULL) {
			MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("%s():no memory for MIC calculation!\n",
					 __func__));
			RELEASE_NDIS_PACKET(pAd, pTxBlk->pPacket, NDIS_STATUS_FAILURE);
			return NDIS_STATUS_FAILURE;
		}
...
INT sta_mlme_mgmtq_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *tx_blk)
{
	...
	*/
	...
	if ((pHeader_802_11->FC.SubType == SUBTYPE_DATA_NULL) || (pHeader_802_11->FC.SubType == SUBTYPE_QOS_NULL)) {
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) && RTMPEnterPsmNullBitStatus(&sta_cfg->PwrMgmt)) {
			/* PCI use Miniport to send NULL frame and need to add NULL frame TxS control here to enter PSM */
			mac_info.PID = (pHeader_802_11->FC.PwrMgmt == PWR_ACTIVE) ? PID_NULL_FRAME_PWR_ACTIVE : PID_NULL_FRAME_PWR_SAVE;
			RTMPClearEnterPsmNullBit(&sta_cfg->PwrMgmt);
...
INT sta_mlme_dataq_tx(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *tx_blk)
{
	...
	*/
	...
	if ((pHeader_802_11->FC.Type == FC_TYPE_DATA) &&
		...
		if ((!RTMP_TEST_FLAG(pAd, fRTMP_ADAPTER_BSS_SCAN_IN_PROGRESS)) && RTMPEnterPsmNullBitStatus(&pStaCfg->PwrMgmt)) {
...
INT sta_tx_pkt_handle(RTMP_ADAPTER *pAd, struct wifi_dev *wdev, TX_BLK *pTxBlk)
{
	...
	if (pStaCfg->PwrMgmt.bDoze)
#else
#if defined(STA_LP_PHASE_2_SUPPORT)
	if (pStaCfg->PwrMgmt.bDoze &&
...
#endif
	{
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_INFO, ("%s::AsicWakeup At HardTx\n", __func__));
		RTMP_FORCE_WAKEUP(pAd, pStaCfg);
	}
	...
	if ((!(pStaCfg->wdev.UapsdInfo.bAPSDCapable && pAd->CommonCfg.APEdcaParm[0].bAPSDCapable)
		...
		if ((RtmpPktPmBitCheck(pAd, pStaCfg) == TRUE) && (pStaCfg->WindowsPowerMode == Ndis802_11PowerModeFast_PSP))
	...
	default:
		MTWF_LOG(DBG_CAT_CLIENT, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Send a pacekt was not classified!!\n"));
...
BOOLEAN sta_dev_rx_mgmt_frm(RTMP_ADAPTER *pAd, RX_BLK *pRxBlk, MAC_TABLE_ENTRY *pEntry)
{
	...
	for (i = 0; i < pAd->MSTANum; i++) {
		...
		if ((FC->SubType == SUBTYPE_BEACON)
			...
			if (pAd->RxAnt.EvaluatePeriod == 0) {
				Update_Rssi_Sample(pAd, &pAd->StaCfg[i].RssiSample, &pRxBlk->rx_signal, pRxBlk->rx_rate.field.MODE, pRxBlk->rx_rate.field.BW);
	...
	if ((FC->SubType == SUBTYPE_BEACON) && (ADHOC_ON(pAd)) && pEntry)
		Update_Rssi_Sample(pAd, &pEntry->RssiSample, &pRxBlk->rx_signal, pRxBlk->rx_rate.field.MODE, pRxBlk->rx_rate.field.BW);
