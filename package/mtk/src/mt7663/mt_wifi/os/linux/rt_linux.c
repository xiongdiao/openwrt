...
BOOLEAN RTMPL2FrameTxAction(
	IN VOID *pCtrlBkPtr,
	IN PNET_DEV pNetDev,
	IN RTMP_CB_8023_PACKET_ANNOUNCE_CMM _announce_802_3_packet_cmm,
	IN UCHAR apidx,
	IN UCHAR *pData,
	IN UINT32 data_len,
	IN UCHAR OpMode)
{
	struct _RTMP_ADAPTER *pAd = (struct _RTMP_ADAPTER *)pCtrlBkPtr;
	...
	if (_announce_802_3_packet_cmm)
		_announce_802_3_packet_cmm(pAd, wdev, skb);
	return TRUE;
}
...
static INT oom_handler(struct notifier_block *this, unsigned long ev, void *ptr)
{
	...
	for (i = 0; i < OOM_NOTIFIER_DUMP_CNT; i++) {
		MTWF_LOG(DBG_CAT_AP, DBG_SUBCAT_ALL, DBG_LVL_ERROR, ("Round ##### %d #####\n", i));
		wifi_dump_info();
	}
