...
#if defined(RLM_CAL_CACHE_SUPPORT) || defined(PRE_CAL_TRX_SET2_SUPPORT) || defined(PRE_CAL_MT7622_SUPPORT) || defined(PRE_CAL_MT7663_SUPPORT) || defined(PRE_CAL_MT7626_SUPPORT)
						 ...
						 UINT32 item, UINT32 band_idx)
{
	...
	if (IS_MT7615(pAd) || IS_MT7663(pAd) || IS_MT7626(pAd)) {
		if ((IS_MT7615(pAd) || IS_MT7626(pAd)) && (item == CAL_ALL))
						 ...
						 UINT32 rdd_rx_sel, UINT32 is_start)
{
	...
	if (IS_MT7615(pAd) || IS_MT7636(pAd) || IS_MT7637(pAd) || IS_MT7626(pAd)) {
...
INT32 MtCmdExtPmStateCtrl(RTMP_ADAPTER *pAd, MT_PMSTAT_CTRL_T PmStatCtrl)
{
	...
	if ((PmStatCtrl.PmNumber == PM4) && (PmStatCtrl.PmState == ENTER_PM_STATE)) {
		 ...
		 */
#ifdef STA_LP_PHASE_1_SUPPORT
		/* MAC_IO_READ32(pAd->hdev_ctrl, RMAC_RFCR_BAND_0, &u4RfCr); */
		u4TempRfCr = u4RfCr;
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s::DbdcIdx(%d), WlanIdx(%d), OwnMacIdx(%d, %d), (%x, %x, %x, %x, %x, %x), BeaconPeriod(%d), DtimPeriod(%d), AID(%d), Feature(0x%x)\n",
			  __func__,
...
INT32 MtCmdChannelSwitch(RTMP_ADAPTER *pAd, MT_SWITCH_CHANNEL_CFG SwChCfg)
{
	...
	if (pAd->CommonCfg.dbdc_mode) {
		if (SwChCfg.BandIdx == DBDC_BAND0) {
			...
			CmdChanSwitch.ucTxStreamNum = pAd->dbdc_band0_tx_path;
			CmdChanSwitch.ucRxStreamNum = pAd->dbdc_band0_rx_path;
		} else {
			ucTxPath = pAd->dbdc_band1_tx_path;
			CmdChanSwitch.ucTxStreamNum = pAd->dbdc_band1_tx_path;
			CmdChanSwitch.ucRxStreamNum = pAd->dbdc_band1_rx_path;
		}
	}
#endif
...
static VOID MtCmdGetTxPowerRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("%s, Power: 0x%x Band: %d\n",
...
INT32 MtCmdSetTxPowerCtrl(RTMP_ADAPTER *pAd, ATE_TXPOWER TxPower)
{
	...
	CmdTxPwrCtrl.u1PowerCtrlFormatId = TX_POWER_SET_TARGET_POWER;
...
INT32 MtCmdGetTxPower(RTMP_ADAPTER *pAd, UINT8 u1DbDcIdx, UINT8 u1CenterCh, UINT8 u1AntIdx, P_EXT_EVENT_ID_GET_TX_POWER_T prTxPwrResult)
{
	...
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: u1CenterCh: %d, u1BandIdx: %d, u1AntIdx: %d\n", __func__, u1CenterCh, u1DbDcIdx, u1AntIdx));
	...
	CmdTxPwrGetCtrl.u1PowerCtrlFormatId = TX_POWER_GET_TARGET_POWER;
...
{
	struct cmd_msg *msg;
	struct _EXT_CMD_PEER_AID_T peer_aid;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: Aid = %d\n", __func__, Aid));
	...
	peer_aid.cmd_category_id = BF_AID_SET;
	peer_aid.wlan_idx = 0; /* not use*/
	peer_aid.aid = Aid;
	if (ATE_ON(pAd)) {
		...
		if (control_band_idx == TESTMODE_BAND1) /* avoid build error */
			wdev_idx = TESTMODE_GET_PARAM(ATECtrl, TESTMODE_BAND1, wdev_idx);
		else
			wdev_idx = TESTMODE_GET_PARAM(ATECtrl, TESTMODE_BAND0, wdev_idx);
		wdev = pAd->wdev_list[wdev_idx];
		peer_aid.omac_idx = HcGetOmacIdx(pAd, wdev);/* only use in ate mode*/
	} else {
		peer_aid.omac_idx = OmacIdx;
	}
#ifdef RT_BIG_ENDIAN
	peer_aid.aid = cpu2le16(peer_aid.aid);
#endif
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	...
	SET_CMD_ATTR_RSP_HANDLER(attr, EventExtCmdResult);
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)&peer_aid, sizeof(peer_aid));
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s:(ret = %d)\n", __func__, ret));
	...
	BOOLEAN      fgBFer)
{
	...
	if (ATECtrl->Mode & ATE_START)
		ETxBfPfmuProfileTag.ucBandIdx = ATECtrl->control_band_idx;
	else
#endif/* CONFIG_ATE */
		ETxBfPfmuProfileTag.ucBandIdx = BandIdx;

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: PFMU ID = %d, BFer: %x, Band:%d\n", __func__,
	...
	UCHAR        PfmuIdx)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: PFMU ID = %d, Band:%d\n", __func__, ETxBfPfmuProfileTag.ucPfmuIdx, ETxBfPfmuProfileTag.ucBandIdx));
...
#ifdef CONFIG_ATE
	if (ATECtrl->Mode & ATE_START)
		ETxBfPfmuProfileData.ucBandIdx = ATECtrl->control_band_idx;
	else
#endif/* CONFIG_ATE */
		ETxBfPfmuProfileData.ucBandIdx = BandIdx;

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: PFMU ID = %d, BFer:%x, Subcarrier:%d, Band:%d\n", __func__,
	...
	PUCHAR       pProfileData)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: PFMU ID = %d, Subcarrier:%d, Band:%d\n", __func__,
...
#ifdef CONFIG_ATE
	if (ATECtrl->Mode & ATE_START)
		ETxBfPfmuProfilePn.ucBandIdx = ATECtrl->control_band_idx;
	else
#endif/* CONFIG_ATE */
...
#ifdef CONFIG_ATE
	if (ATECtrl->Mode & ATE_START)
		ETxBfQdData.ucBandIdx = ATECtrl->control_band_idx;
	else
#endif/* CONFIG_ATE */
		ETxBfQdData.ucBandIdx = BandIdx;

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: subCarrIdx = %d,  Band:%d\n", __func__, ETxBfQdData.cSubCarr, ETxBfQdData.ucBandIdx));
...
INT32 CmdETxBfFbRptDbgInfo(
	PRTMP_ADAPTER pAd,
	P_EXT_CMD_TXBF_FBRPT_DBG_INFO_T prETxBfFbRptData)
{
	struct cmd_msg *msg;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	prETxBfFbRptData->ucCmdCategoryID = BF_FBRPT_DBG_INFO_READ;
#ifdef CONFIG_ATE
	if (ATECtrl->Mode & ATE_START)
		prETxBfFbRptData->ucBandIdx = ATECtrl->control_band_idx;
	else
#endif/* CONFIG_ATE */
		prETxBfFbRptData->ucBandIdx = BandIdx;

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s: ucAction:%u, BandIdx:%u, PollPFMUIntrStatTimeOut:%u, FbRptDeQInterval =%u, ucWlanIdx:%u, PFMUUpdateEn:%u\n", __func__
			 , prETxBfFbRptData->ucAction, prETxBfFbRptData->ucBandIdx
			 , prETxBfFbRptData->ucPollPFMUIntrStatTimeOut
			 , prETxBfFbRptData->ucFbRptDeQInterval
			 , prETxBfFbRptData->ucWlanIdx
			 , prETxBfFbRptData->ucPFMUUpdateEn));
	...
	UINT8 mcsth)
{
	struct cmd_msg *msg;
	INT32 ret = 0;
	struct dynsnd_cfg_dmcsth_info cmd_info;
	struct _CMD_ATTRIBUTE attr = {0};

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s: mcs_index = %d, mcsth = %d\n", __func__, mcs_index, mcsth));
	msg = MtAndesAllocCmdMsg(pAd, sizeof(cmd_info));
	...
	UINT8 pfid)
{
	struct cmd_msg *msg;
	INT32 ret = 0;
	struct dynsnd_en_mu_intr_info cmd_info;
	struct _CMD_ATTRIBUTE attr = {0};

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_ERROR,
			 ("%s: mu_intr_en = %d, pfid = %d\n", __func__, mu_intr_en, pfid));
	msg = MtAndesAllocCmdMsg(pAd, sizeof(cmd_info));
...
VOID MtCmdEfusBufferModeSet(RTMP_ADAPTER *pAd, UINT8 EepromType)
{
	...
	CmdEfuseBufferMode->ucContentFormat = CONTENT_FORMAT_WHOLE_CONTENT;
#endif
#if defined(MT7663)
	/* bit 5 : Trigger command, it doesn't need to power-on cal */
	if (EepromType == EEPROM_FLASH_CMD)
		CmdEfuseBufferMode->ucContentFormat |= BIT(5);
#endif
#ifdef RT_BIG_ENDIAN
...
#if defined(MT7663) || defined(MT7626)
	CmdEfuseBufferModeRead->ucContentFormat = CONTENT_FORMAT_WHOLE_CONTENT;
#endif
#ifdef RT_BIG_ENDIAN
...
)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
		("%s: ucBand:%d, HighEn:%d, HighTempTh:%d, LowEn:%d, LowTempTh:%d, RechkTimer:%d\n", __func__,
		ucBand, HighEn, HighTempTh, LowEn, LowTempTh, RechkTimer));

	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
				 ("%s: RFOffEn: %d, RFOffTh: %d, ucType: %d\n", __func__, RFOffEn, RFOffTh, ucType));
...
#ifdef THERMAL_PROTECT_SUPPORT
INT32
MtCmdThermalProtectAdmitDutyInfo(
	RTMP_ADAPTER *pAd
)
{
	struct cmd_msg *msg;
...
INT32 MtCmdGetThermalSensorResult(RTMP_ADAPTER *pAd, UINT8 ActionIdx, UINT8 ucDbdcIdx, UINT32 *SensorResult)
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_OFF,
			 ("%s: ActionIdx: %d, uBandIdx: %d\n", __func__, ActionIdx, ucDbdcIdx));
	...
	ThermalSensorInfo.u1ThermalCtrlFormatId = THERMAL_SENSOR_INFO_GET;
...
static VOID MtCmdGetTxStatisticRsp(struct cmd_msg *msg, char *Data, UINT16 Len)
{
	...
	if (prTxStat->u4Field & GET_TX_STAT_ENTRY_TX_PER) {
		prTxStat->ucEntryTxPer =
			le2cpu32(prEventExtCmdResult->ucEntryTxPer);
	}
}
...
INT32 MtCmdBcnV2OffloadSet(RTMP_ADAPTER *pAd, CMD_BCN_OFFLOAD_T_V2 *bcn_offload_v2)
{
	...
	MTWF_LOG(DBG_CAT_ALL, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s(): Enable = %d, OwnMacIdx = %d, WlanIdx = %d, Band = %d, Len = %d\n",
			  __func__, bcn_offload_v2->ucEnable, bcn_offload_v2->ucOwnMacIdx,
...
INT32 MtCmdSetFreqOffset(RTMP_ADAPTER *pAd, UINT32 FreqOffset, UINT8 BandIdx)
{
	...
	os_zero_mem(&ATE_param, sizeof(ATE_param));
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s, BandIdx:%d, FreqOffset:%d\n", __func__, BandIdx, FreqOffset));
	ATE_param.ucAteTestModeEn = testmode_en;
...
INT32 MtCmdSetGroupPreCal_7663(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	struct cmd_msg *msg;
	...
	Len = GroupPreCalInfoAlloc_7663(pAd, &pCmdGroupPreCalInfo, idx, length);
...
INT32 MtCmdSetDpdFlatnessCal_7663(RTMP_ADAPTER *pAd, UINT16 idx, UINT32 length)
{
	struct cmd_msg *msg;
	...
	Len = DpdFlatnessCalInfoAlloc_7663(pAd, &pCmdDpdFlatnessCalInfo, idx, length);
...
)
{
	...
	msg = MtAndesAllocCmdMsg(pAd, sizeof(CMD_POWER_LIMIT_TABLE_CTRL_T));
	...
	SET_CMD_ATTR_RSP_HANDLER(attr, NULL);
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)&rPwrLimitTblCtrl,
...
{
	struct cmd_msg *msg;
	CMD_TEMPERATURE_CTRL_T TemperatureCtrl;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	...
	msg = MtAndesAllocCmdMsg(pAd, sizeof(CMD_TEMPERATURE_CTRL_T));
	...
	os_zero_mem(&TemperatureCtrl, sizeof(CMD_TEMPERATURE_CTRL_T));
	TemperatureCtrl.u1PowerCtrlFormatId = THERMAL_MANUAL_CTRL;
	TemperatureCtrl.fgManualMode        = fgManualMode;
	TemperatureCtrl.i1Temperature       = cTemperature;
	TemperatureCtrl.u1BandIdx           = ucDbdcBandIdx;
	SET_CMD_ATTR_MCU_DEST(attr, HOST2N9);
	SET_CMD_ATTR_TYPE(attr, EXT_CID);
	SET_CMD_ATTR_EXT_TYPE(attr, EXT_CMD_ID_THERMAL_FEATURE_CTRL);
	SET_CMD_ATTR_CTRL_FLAGS(attr, INIT_CMD_SET);
...
INT32 MtCmdThermalItemInfo(
	RTMP_ADAPTER *pAd
	)
{
	struct cmd_msg *msg;
	CMD_THERMAL_ITEM_INFO_T ThermalItemInfo;
	INT32 ret = 0;
	struct _CMD_ATTRIBUTE attr = {0};
	msg = MtAndesAllocCmdMsg(pAd, sizeof(CMD_THERMAL_ITEM_INFO_T));
	...
	os_zero_mem(&ThermalItemInfo, sizeof(CMD_THERMAL_ITEM_INFO_T));
	ThermalItemInfo.u1PowerCtrlFormatId = THERMAL_SENSOR_BASIC_INFO;
	...
	SET_CMD_ATTR_RSP_HANDLER(attr, NULL);
	MtAndesInitCmdMsg(msg, attr);
	MtAndesAppendCmdMsg(msg, (char *)&ThermalItemInfo,
						sizeof(CMD_THERMAL_ITEM_INFO_T));
	ret = chip_cmd_tx(pAd, msg);
error:
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			 ("%s:(ret = %d)\n", __func__, ret));
	...
	CHAR          cPwrUpValue[POWER_UP_CATEGORY_RATE_NUM])
{
	...
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
		("%s: cPwrUpValue:", __func__));
	for (u1idx = 0; u1idx < POWER_UP_CATEGORY_RATE_NUM; u1idx++) {
		MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE,
			(" -(%d)", cPwrUpValue[u1idx]));
		}
	MTWF_LOG(DBG_CAT_FW, DBG_SUBCAT_ALL, DBG_LVL_TRACE, ("\n"));
