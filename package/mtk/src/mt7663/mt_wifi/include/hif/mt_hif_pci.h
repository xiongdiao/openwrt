...
struct hif_pci_ring_desc {
	UINT8 hw_dma_id;
	UINT8 hw_ring_id;
	UINT32 hw_desc_base;
	UINT16 ring_size;
	enum ring_attr ring_attr;
};

struct hif_pci_ring_layout {
	const struct hif_pci_ring_desc *tx_ring_layout;
	const struct hif_pci_ring_desc *rx_ring_layout;
};
...
#define MAX_HW_DMA_NUM	2
#define MAX_HW_TX_RING_NUM 20
#define MAX_HW_RX_RING_NUM 4
...
typedef struct _PCI_HIF_T {
	...
	UINT8 hw_to_tx_ring[MAX_HW_DMA_NUM][MAX_HW_TX_RING_NUM]; /* map HW dma id and ring id to SW TxRing resource_idx */
	UINT8 hw_to_rx_ring[MAX_HW_DMA_NUM][MAX_HW_RX_RING_NUM]; /* map HW dma id and ring id to SW RxRing resource_idx */
	UINT8 swq_to_tx_ring[PACKET_TYPE_NUM][WMM_QUE_NUM]; /* map swq to SW TxRing resource_idx */
